...
$SelfName="Invoke-SGODA-SPT02411-Capa2-FINAL-v1.0.3-PS51.ps1"
...
function SizeGate(){
  ...
  if($LASTEXITCODE -ne 0){throw "Unable to enumerate Git index for size gate"}
...
if($Deleted.Count -ne 0){Hold "Unsafe pre-existing tracked deletions detected"}

if($Staged.Count -gt 0){
  $AllowedRecoveryMasters=@(
    "Invoke-SGODA-SPT02411-Capa2-FINAL-v1.0.0-PS51.ps1",
    "Invoke-SGODA-SPT02411-Capa2-FINAL-v1.0.1-PS51.ps1",
    "Invoke-SGODA-SPT02411-Capa2-FINAL-v1.0.2-PS51.ps1"
  )
  $UnexpectedPreStaged=@($Staged|Where-Object{
    $n=($_ -replace '\\','/')
    -not(
      $n -eq $TestFile -or
      $n -eq $PolicyFile -or
      $n -eq $DocFile -or
      $n.StartsWith("$ModuleDir/") -or
      $n.StartsWith("$ArtifactDir/") -or
      $AllowedRecoveryMasters -contains $n
    )
  })

  if($UnexpectedPreStaged.Count -gt 0){
    $UnexpectedPreStaged | ForEach-Object { Write-Host "UNEXPECTED PRE-STAGED : $_" -ForegroundColor Red }
    Hold "Unsafe pre-existing staged state outside SPT-024.11 Capa 2 recovery scope"
  }

  Write-Host "CONTROLLED PRE-STAGED RECOVERY : ACCEPTED ($($Staged.Count) paths)"
  & git.exe restore --staged -- $Staged
  if($LASTEXITCODE -ne 0){Hold "Unable to safely recover controlled staging"}

  $RemainingStaged=@(& git.exe -c core.quotepath=false diff --cached --name-only)
  if($RemainingStaged.Count -ne 0){Hold "Controlled staging recovery did not leave the index clean"}

  Write-Host "STAGING RECOVERY : PASS"
  Write-Host "WORKING TREE FILES : PRESERVED"
}

Write-Host "BASELINE : PASS"
Write-Host "SPT-024.1-.10 + SPT-024.11 CAPA 1 : PROTECTED / NOT REOPENED"
...
if(Test-Path -LiteralPath $FailedMaster){
  Write-Host "FAILED MASTERS v1.0.0-v1.0.2 : LOCAL / SUPERSEDED / NOT PUBLISHED"
}
Write-Host "RECOVERY MASTER v1.0.3  : ACTIVE"
...
Write-Host ""; Write-Host "SPT-024.11 CAPA 2 : TECHNICALLY CLOSED / RECOVERY CERTIFIED v1.0.3" -ForegroundColor Green
