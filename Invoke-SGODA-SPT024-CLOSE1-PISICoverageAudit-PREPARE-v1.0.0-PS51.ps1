...
$ArtifactDir="artifacts/development/SPT-024.CLOSE.1-v1.0.0"
...
if($Existing.Count-gt0){Write-Host "CLOSE.1 RESUME MODE : ACTIVE"}else{Write-Host "CLOSE.1 FRESH IMPLEMENTATION : ACTIVE"}
...
def component_from_path(path):
    m = re.search(r"SPT-024\.(1[0-7]|[1-9])(?!\d)", path, re.I)
    return f"SPT-024.{int(m.group(1))}" if m else None
...
'@
$Init=@'
from .core import EXPECTED, build_coverage, summarize
__all__=["EXPECTED","build_coverage","summarize"]
'@
$Tests=@'
...
WriteLf $EvidenceFile ([ordered]@{component="SPT-024.CLOSE.1";version="1.0.0";baseline=$ExpectedBaseline;status="PISI_GLOBAL_PREPARE_GATE_PASS";targeted_tests="PASS";institutional_suite="PASS";compileall="PASS";closed_components_preserved=$true;local_remote_sync_required=$true}|ConvertTo-Json -Depth 8)
...
$Allowed=@("Invoke-SGODA-SPT024-CLOSE1-PISICoverageAudit-PREPARE-v1.0.0-PS51.ps1",$CoreFile,$InitFile,$TestFile,$PolicyFile,$DocFile,$CoverageFile,$ControlMatrixFile,$ReconciliationFile,$AssessmentFile,$IntegrityFile,$PrepareFile,$EvidenceFile)
...
Step 14 "COMMIT"
& git.exe commit -m "feat(spt-024.close.1): audit PISI coverage reconcile domains and prepare institutional closure"
