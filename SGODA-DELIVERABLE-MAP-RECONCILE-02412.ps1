...
$ExpectedBaseline = "02a1485b02130834909e83a26321440ed78c0a80"
$Branch = "feature/SPT-001A-rlb-schema-foundation"
$Version = "1.0.1"
$Self = "SGODA-DELIVERABLE-MAP-RECONCILE-02412.ps1"

$PrepareScript = "SGODA-SPT02412-FORMALIZE-PREPARE.ps1"
$PrepareDev = "artifacts/development/SPT-024.12-FORMALIZE-PREPARE-v1.0.0"
$PrepareAssessment = "$PrepareDev/spt02412-real-state-assessment.json"
$PrepareCoverage = "$PrepareDev/spt02412-coverage-matrix.json"
$PrepareEvidence = "$PrepareDev/spt02412-evidence-inventory.json"
$PrepareGit = "$PrepareDev/spt02412-commits-tags-releases.json"
$PrepareContract = "$PrepareDev/spt02412-formalization-prepare.json"
$PrepareIntegrity = "$PrepareDev/spt02412-prepare-sha256-manifest.json"
$PrepareImplementationEvidence = "$PrepareDev/implementation-evidence.json"
$PrepareDoc = "docs/06_Tecnologia/SPT-024/SPT-024.12/SGD-SPT024.12-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$PreviousMapDir = "artifacts/development/SGODA-DELIVERABLE-MAP-UPDATE-v1.0.0"
$PreviousInventory = "$PreviousMapDir/global-deliverable-inventory.json"
$PreviousStatus = "$PreviousMapDir/global-deliverable-status-matrix.json"
$PreviousNext = "$PreviousMapDir/next-technological-deliverable-assessment.json"
$PreviousMapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.0.md"

$Dev = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1"
...
$ReconciliationAssessment = "$Dev/spt02412-map-reconciliation-assessment.json"
...
$MapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.1.md"
$Acta = "docs/00_Estado_Maestro/ACT-SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-02412 : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 2 "CONSUME SPT-024.12 FORMALIZE PREPARE"
    $PrepareFiles = @(
        $PrepareScript,
        $PrepareAssessment,
        $PrepareCoverage,
    ...
    if([string]$PrepareState.real_state -ne "ALREADY_CLOSED"){ Hold "SPT-024.12 PREPARE does not certify ALREADY_CLOSED" }
    if([string]$PrepareState.recommended_action -ne "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"){ Hold "Unexpected PREPARE recommended action" }

    Write-Host "SPT02412_PREPARE=PASS"
    Write-Host "SPT02412_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT02412_RECOMMENDED_ACTION=RECONCILE_MASTER_MAP_WITHOUT_REOPENING"

    Step 3 "CONSUME PREVIOUS GLOBAL DELIVERABLE MAP"
    foreach($P in @($PreviousInventory,$PreviousStatus,$PreviousNext,$PreviousMapDoc)){
    ...
    Write-Host "PREVIOUS_NEXT_CANDIDATE=$([string]$Next.next_deliverable_candidate)"
    Write-Host "PREVIOUS_MAP_CONSUMED=PASS"

    Step 4 "RECLASSIFY SPT-024.12 WITHOUT REOPENING"
    $Rows = @($Inventory.deliverables)
    $Target = @($Rows | Where-Object { [string]$_.deliverable -eq "SPT-024.12" })

    if($Target.Count -ne 1){ Hold ("Expected exactly one SPT-024.12 inventory row, found "+$Target.Count) }
    ...
    foreach($Row in $Rows){
        ...
        if($Id -eq "SPT-024.12"){
            $ReconciledRows += [ordered]@{
                deliverable = "SPT-024.12"
                status = "CLOSED"
                reason = "Recertified by SPT-024.12.FORMALIZE.PREPARE as ALREADY_CLOSED; master map corrected without reopening."
    ...
    Write-Host "SPT02412_PREVIOUS_CLASSIFICATION=$PreviousTargetStatus"
    Write-Host "SPT02412_CLASSIFICATION=CLOSED"
    Write-Host "SPT02412_REOPENED=NO"
    ...
    if($PendingIds -contains "SPT-024.12"){ Hold "SPT-024.12 remains in pending candidates after reconciliation" }
    ...
    Write-Host "SPT02412_PENDING_AFTER_RECONCILIATION=NO"
    ...
    Step 7 "RECONCILE TAGS / RELEASES / COMMITS"
    $RecentCommits = @(& git.exe --no-pager log --format="%H|%s" -50)
    $Tags = @(& git.exe tag --list)
    $Tags02412 = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?024[-_.]?12|SPT02412' })
    $Tracked = @(& git.exe -c core.quotepath=false ls-files)
    $Release02412 = @($Tracked | Where-Object { $_ -match '(?i)^releases/.*SPT[-_.]?024[-_.]?12|^releases/.*SPT02412' })

    Write-Host "RECENT_COMMITS=$($RecentCommits.Count)"
    Write-Host "SPT02412_TAGS=$($Tags02412.Count)"
    Write-Host "SPT02412_RELEASE_PATHS=$($Release02412.Count)"
    ...
    $InventoryObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-02412"
        version=$Version
    ...
    $StatusObj=[ordered]@{
        ...
        spt02412_classification="CLOSED"
        spt02412_reopened=$false
        classification_policy="Repository evidence plus explicit SPT-024.12 PREPARE recertification."
    }
    ...
    $NextObj=[ordered]@{
        ...
        spt02412_removed_from_pending=$true
    }
    ...
    $AssessmentObj=[ordered]@{
        ...
        spt02412_real_state="ALREADY_CLOSED"
        spt02412_previous_map_status=$PreviousTargetStatus
        spt02412_reconciled_status="CLOSED"
        spt02412_reopened=$false
    ...
    $LedgerObj=[ordered]@{
        ...
        prepare_inputs=$PrepareFiles
        previous_map_inputs=@($PreviousInventory,$PreviousStatus,$PreviousNext,$PreviousMapDoc)
        correction=[ordered]@{
            deliverable="SPT-024.12"
            from=$PreviousTargetStatus
            to="CLOSED"
            reopen=$false
            justification="ALREADY_CLOSED certified by formalization PREPARE."
        }
    ...
    $EvidenceObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-02412"
        authoritative_input_head=$ExpectedBaseline
        spt02412_prepare_consumed=$true
        spt02412_reopened=$false
    ...
    $Doc=@"
# SGODA-PUINAVE - Mapa Global de Entregables v1.0.1

Linea base: $ExpectedBaseline

## Reconciliacion SPT-024.12
...
SPT-024.12 no se reabre. La correccion modifica exclusivamente la clasificacion institucional del mapa y publica la evidencia PREPARE que demuestra el estado real.
"@

    $ActaText=@"
# ACT-SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1

Se certifica la reconciliacion del Mapa Global de Entregables para SPT-024.12.

- Baseline de entrada: $ExpectedBaseline
- SPT-024.12 real state: ALREADY_CLOSED
- SPT-024.12 map classification: CLOSED
- SPT-024.12 reopened: NO
...
"@
    ...
    Write-Host "MAP_DOCUMENT_V1.0.1=CREATED"
    ...
    $BlockingUnexpected=@($UnexpectedUntracked | Where-Object {
        $_ -match '(?i)SPT[-_.]?024[-_.]?12|SPT02412'
    })

    if($BlockingUnexpected.Count -ne 0){
        $BlockingUnexpected | ForEach-Object { Write-Host ("BLOCKING_UNTRACKED="+$_) }
        Hold "Unexpected active SPT-024.12 content outside publication set"
    }

    Write-Host "CLOSED_BASELINE_PRESERVED=PASS"
    Write-Host "SPT02412_UNTRACKED_ACCOUNTING=PASS"
    ...
    if($RemotePre -ne $ExpectedBaseline -or $LocalPre -ne $ExpectedBaseline){ Hold "Remote changed before reconciliation commit" }

    Write-Host "GITHUB_SIZE_GATE=PASS"
    Write-Host "REMOTE_PRECOMMIT_GATE=PASS"

    Step 14 "COMMIT MAP RECONCILIATION"
    & git.exe commit -m "chore(institutional): reconcile SPT-024.12 as already closed"
    ...
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-02412 : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT02412_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT02412_CLASSIFICATION=CLOSED"
    Write-Host "SPT02412_REOPENED=NO"
