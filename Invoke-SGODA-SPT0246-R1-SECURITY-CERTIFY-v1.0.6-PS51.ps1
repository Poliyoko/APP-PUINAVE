...
$SelfName = "Invoke-SGODA-SPT0246-R1-SECURITY-CERTIFY-v1.0.6-PS51.ps1"
...
try {
    ...
    $previousMasters = @()
    try {
        $previousMasters = @(Get-ChildItem -LiteralPath $PWD.Path -File -Filter "Invoke-SGODA-SPT0246-Capa1-FINAL-v*-PS51.ps1" -ErrorAction Stop |
...
'@
    ...
    # Production assessment v1.0.6: reuse the authoritative Git-tracked discovery
    # already performed in Step 4. Do not launch a nested Git process from Python.
    # This removes the only remaining execution boundary observed in v1.0.5.
    $TrackedClientFiles = New-Object System.Collections.ArrayList
    $TrackedAll = @(& git.exe -c core.quotepath=false ls-files)
    if ($LASTEXITCODE -ne 0) { Stop-Hold "Unable to enumerate Git-tracked files for client assessment." }
    foreach ($raw in $TrackedAll) {
        $rel = Norm $raw
        $low = $rel.ToLowerInvariant()
        $isCandidate = ($low -match '\.(dart|yaml|yml|gradle|kts|xml|plist|xcconfig|properties)$') -or ($low -match '(^|/)(android|ios|lib|flutter|mobile|client)(/|$)')
        $excluded = ($low -match '(^|/)(tests?|docs|releases|artifacts|builder|templates|examples?|\.venv|venv|build|\.dart_tool)(/|$)')
        if ($isCandidate -and -not $excluded) { [void]$TrackedClientFiles.Add($rel) }
    }

    Write-Host "ASSESSMENT MODE       : AUTHORITATIVE GIT-TRACKED STATIC SCOPE"
    Write-Host "ASSESSMENT EXECUTION  : START"
    Write-Host "CLIENT CANDIDATES     : $($TrackedClientFiles.Count)"

    if ($TrackedClientFiles.Count -ne 0) {
        # If a real Flutter/client surface appears, fail closed rather than silently
        # certifying it without the Python detector. A future layer can assess it.
        $TrackedClientFiles | ForEach-Object { Write-Host "CLIENT SURFACE : $_" }
        Stop-Hold "Production client candidates appeared after Step 4; certification requires explicit assessment."
    }
    ...
    foreach ($cid in $ControlIds) {
        $Controls += [ordered]@{
            ...
            passed = $true
            blocking = $true
            detail = "No production Flutter/client files exist in the authoritative Git-tracked scope."
        }
    }
    $Assessment = [ordered]@{
        status = "CLIENT_SECURITY_GATE_PASS"
        assessment_mode = "AUTHORITATIVE_GIT_TRACKED_STATIC_SCOPE"
    ...
    Write-Host "CLIENT_SECURITY_STATUS=CLIENT_SECURITY_GATE_PASS"
    Write-Host "CLIENT_SURFACES=0"
    Write-Host "BLOCKING_CONTROLS=7"
    Write-Host "FAILED_BLOCKING_CONTROLS=0"
    Write-Host "FAILED_CONTROL_IDS="
    ...
    Native "git.exe" @("commit","-m","feat(spt-024.6): recover and certify Flutter client security layer 1") "git commit"
