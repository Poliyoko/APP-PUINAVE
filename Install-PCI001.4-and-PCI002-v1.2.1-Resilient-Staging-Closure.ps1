<#
.SYNOPSIS
    Instala y ejecuta PCI-002 v1.2.1 — Institutional Consolidation Engine — Resilient Staging Closure.
...
function Get-ShortStagingBase {
    ...
    throw "No se pudo crear una ubicación temporal escribible."
}

function Invoke-RobocopyAttempt {
    param(
        [string]$Source,
        [string]$Destination,
        [string]$LogPath,
...
function Copy-RepositoryToStage {
    ...
    $LogDirectory = Join-Path `
        $ProjectRoot `
        "artifacts\consolidation\PCI-002-v1.2.1\robocopy"
    New-Item `
        -ItemType Directory `
        -Path $LogDirectory `
        -Force |
    ...
    $FirstLog = Join-Path $LogDirectory "staging-copy-attempt-1.log"
    $SecondLog = Join-Path $LogDirectory "staging-copy-attempt-2.log"

    $Code = Invoke-RobocopyAttempt `
        -Source $Source `
    ...
    if ($Code -ge 8) {
        Write-Host (
            "Robocopy intento 1 terminó con código " +
            [string]$Code +
            ". Ejecutando reintento reforzado."
        ...
        $Code = Invoke-RobocopyAttempt `
            ...
            -Destination $Destination `
            -LogPath $SecondLog `
            -RestartableBackupMode
    }

    if ($Code -ge 8) {
        $DiagnosticLog = if (
            Test-Path -LiteralPath $SecondLog
        ) {
            $SecondLog
        }
        else {
        ...
        Write-Host "" 
        Write-Host "Últimas líneas del diagnóstico Robocopy:" -ForegroundColor Red
        ...
        throw (
            "No se pudo construir el área temporal después de dos intentos. " +
            "Robocopy: " +
            [string]$Code +
            ". Diagnóstico: " +
            $DiagnosticLog
    ...
    foreach ($RelativePath in $RequiredStageFiles) {
        $StagePath = Join-Path $Destination $RelativePath

        if (-not (Test-Path -LiteralPath $StagePath -PathType Leaf)) {
            throw (
                "El staging quedó incompleto. Falta: " +
                $RelativePath
            )
        }
    }
    ...
    Write-Host (
        "Área temporal construida correctamente. Robocopy: " +
        [string]$Code
...
$ArtifactDir = Join-Path `
    $ProjectRoot `
    "artifacts\consolidation\PCI-002-v1.2.1"
$BackupDir = Join-Path $ArtifactDir "repository-backup"
$StagingBase = Get-ShortStagingBase
$StageDir = Join-Path `
    $StagingBase `
    (
        "P" +
...
$Architecture = @'
# PCI-002 v1.2.1 — Institutional Consolidation Engine — Resilient Staging Closure
...
try {
    ...
    if ($Publish) {
        ...
        & $PublisherPath `
            -Publish `
            -CommitMessage "feat(consolidation): close PCI-002 v1.2.1 resilient staging closure" `
            -EvidenceCommitMessage "chore(consolidation): publish PCI-002 v1.2.1 evidence"
    ...
    Write-Host "PCI-002 v1.2.1 implementado con PCI-001.3 y PCI-001.4." -ForegroundColor Green
...
Write-Host "Convergencia ciclos 2 y 3: APROBADA." -ForegroundColor Green
    ...
    Write-Host "Release: releases\PCI-002-v1.2.1" -ForegroundColor Cyan
