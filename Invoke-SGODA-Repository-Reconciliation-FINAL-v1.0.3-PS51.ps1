...
$SelfName = "Invoke-SGODA-Repository-Reconciliation-FINAL-v1.0.3-PS51.ps1"
...
function Is-SupersededFailedMaster {
    param([string]$P)

    $p=Norm $P

    $SupersededMap=@{
        "Invoke-SGODA-SPT0242-R1-FINAL-v1.0.0-PS51.ps1" = "Invoke-SGODA-SPT0242-R1-CERTIFY-v1.0.1-PS51.ps1"
        "Invoke-SGODA-SPT0242-Capa1-FINAL-v1.0.0-PS51.ps1" = "Invoke-SGODA-SPT0242-R1-CERTIFY-v1.0.1-PS51.ps1"
        "Invoke-SGODA-SPT0243-Capa1-FINAL-v1.0.1-PS51.ps1" = "Invoke-SGODA-SPT0243-R1-SECURITY-CERTIFY-v1.0.1-PS51.ps1"
        "Invoke-SGODA-SPT0244-Capa1-FINAL-v1.0.0-PS51.ps1" = "Invoke-SGODA-SPT0244-R1-SECURITY-CERTIFY-v1.0.1-PS51.ps1"
        "Invoke-SGODA-SPT0245-Capa1-FINAL-v1.0.0-PS51.ps1" = "Invoke-SGODA-SPT0245-R1-SECURITY-CERTIFY-v1.0.7-PS51.ps1"
    }

    if(-not $SupersededMap.ContainsKey($p)){ return $false }

    $replacement=[string]$SupersededMap[$p]
    & git.exe ls-files --error-unmatch -- $replacement 2>$null | Out-Null
    if($LASTEXITCODE -eq 0){ return $true }

    # If the certified replacement is not tracked, do not suppress the old file.
    return $false
}
...
function Is-InstitutionalCandidate {
    ...
    if(Is-SupersededFailedMaster $p){return $false}
...
try {
    ...
    Write-Host "SUPERSEDED FAILED-MASTER POLICY : v1.0.3 ACTIVE"
    ...
    foreach($record in $Historical){
        ...
        } elseif(Is-SupersededFailedMaster $p){
            $kind="SUPERSEDED_FAILED_MASTER_PRESERVE_NO_PUBLISH"
            $reason="Historical failed master has a certified replacement already tracked in Git; preserve locally, do not republish."
            [void]$ExcludedPaths.Add($p)
    ...
    $SupersededFailedMasters=@(
        $Historical | Where-Object { Is-SupersededFailedMaster (Norm $_.Path) }
    )

    Write-Host "SUPERSEDED FAILED MASTERS : $($SupersededFailedMasters.Count)"
