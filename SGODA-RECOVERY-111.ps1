...
$EvidenceRoot = "artifacts/development/SGODA-InstitutionalMasterSynchronization-RECOVERY-v1.1.1"
...
$RecoveryDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Sincronizacion-Institucional-Maestra-RECOVERY-v1.1.1.md"
$RecoveryAct = "docs/00_Estado_Maestro/ACT-SGODA-MASTER-SYNC-RECOVERY-v1.1.1.md"
...
function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA MASTER SYNCHRONIZATION RECOVERY v1.1.1 : HOLD" -ForegroundColor Red
...
try {
    ...
    foreach($P in $CurrentUntracked) {
        if($P -eq "SGODA-RECOVERY-111.ps1") {
            [void]$Preserve.Add($P)
            continue
        }
    ...
    Write-Host "SUPERSEDED_DOWNLOAD_COPIES=PRESERVED_AS_EVIDENCE"
    ...
    $DocText = @"
# SGODA-PUINAVE - Sincronizacion Institucional Maestra - RECOVERY v1.1.1
...
"@
    $ActText = @"
# ACT-SGODA-MASTER-SYNC-RECOVERY-v1.1.1
...
"@
    ...
    & git.exe commit -m "fix(institutional): complete master synchronization worktree reconciliation recovery v1.1.1"
    ...
    Write-Host "PREPARE_OUTPUT_RECONCILIATION=PASS"
    Write-Host "SUPERSEDED_RECOVERY_COPIES=PRESERVED"
    ...
    Write-Host ""
    Write-Host "SGODA INSTITUTIONAL MASTER SYNCHRONIZATION RECOVERY v1.1.1 : CLOSED / PASS" -ForegroundColor Green
