...
$Version="1.0.1"
$FailedMaster="Invoke-SGODA-InstitutionalMasterSynchronization-SPT025Reconciliation-FINAL-v1.0.0-PS51.ps1"
$MarkerBegin="<!-- SGODA-INSTITUTIONAL-MASTER-SYNCHRONIZATION:v1.0.1:BEGIN -->"
$MarkerEnd="<!-- SGODA-INSTITUTIONAL-MASTER-SYNCHRONIZATION:v1.0.1:END -->"
...
$SyncDocFile="docs/00_Estado_Maestro/SGODA-PUINAVE-Sincronizacion-Institucional-Maestra-v1.0.1.md"

$ArtifactDir="artifacts/development/SGODA-InstitutionalMasterSynchronization-v1.0.1"
...
$ReleaseManifest="releases/SGODA-INSTITUTIONAL-MASTER-SYNC-v1.0.1/manifest.json"
...
function Append-SyncSection{
    ...
    $Current=[IO.File]::ReadAllText($Absolute,[Text.Encoding]::UTF8)
...
try{
    ...
    $CloseAssessment=([IO.File]::ReadAllText((Join-Path $Root $ReqCloseAssessment),[Text.Encoding]::UTF8)|ConvertFrom-Json)
    $CloseManifest=([IO.File]::ReadAllText((Join-Path $Root $ReqCloseManifest),[Text.Encoding]::UTF8)|ConvertFrom-Json)
    ...
    foreach($P in $MutableMasterPaths){
        ...
        $Existing=[IO.File]::ReadAllText($A,[Text.Encoding]::UTF8)
        if($Existing.Contains($MarkerBegin)){Hold ("Synchronization v1.0.1 already applied to "+$P)}
    }
...
'@
    $SyncDocText=@'
# SGODA-PUINAVE - Sincronizacion Institucional Maestra v1.0.1

Linea base fuente obligatoria: `e2deea6e76657d281dbdbec6db338d12b261d585`.

## Proposito

Reconciliar el estado maestro institucional del proyecto con el cierre de SPT-025, actualizar los documentos maestros canonicos y producir una fotografia auditable de trazabilidad, entregables, evidencias, actas, commits, tags y releases.

## Superficies maestras autorizadas para actualizacion
...
Los demas componentes cerrados permanecen congelados por SHA-256.

## Estado reconciliado

SPT-025 se incorpora como programa institucionalmente cerrado, con cobertura y recertificacion 16/16, arquitectura replicable aprobada, SGODA Core compartido y plataformas linguisticas independientes parametrizables.

La sincronizacion no despliega nuevas plataformas ni modifica produccion.
'@
...
"@
    ...
    Append-SyncSection $Sgd002 ($CommonBody+"`nEl Libro Maestro incorpora SPT-025.1-SPT-025.16, CLOSE.1 y CLOSE.2 como linea cerrada y preservada.")
    Append-SyncSection $Rmi ($CommonBody+"`nEl Registro Maestro Institucional incorpora la implementacion replicable SPT-025 y su paquete de cierre.")
    Append-SyncSection $MasterIndex ($CommonBody+"`nIndice actualizado para referenciar el programa SPT-025 cerrado, su acta, ledger, manifiesto y matriz maestra de trazabilidad.")
...
### Convencion de esta operacion

Nombre canonico: **SGODA-INSTITUTIONAL-MASTER-SYNCHRONIZATION**.
Esta operacion no introduce una nueva familia SPT/SPB/SGD; actua como operacion institucional transversal de reconciliacion del estado maestro.
"@
    Append-SyncSection $Nomenclature $NomBody

    $TraceMd="# SGODA-PUINAVE - Matriz Maestra de Trazabilidad Institucional`n`n"
    $TraceMd+="Linea base fuente: ``$ExpectedBaseline``.`n`n"
    $TraceMd+="| Componente | Estado | Commit trazable | Documentacion | Evidencia | Preservado |`n"
    $TraceMd+="|---|---|---|---|---|---|`n"
    ...
    $TraceMd+="`n## Cierre`n`nSPT-025.CLOSE.1 y SPT-025.CLOSE.2 certifican cobertura, recertificacion, ledger, manifiesto global y acta institucional.`n"
    ...
    $Release=[ordered]@{
        release="SGODA-INSTITUTIONAL-MASTER-SYNC-v1.0.1"
    ...
    $Allowed=@(
        "Invoke-SGODA-InstitutionalMasterSynchronization-SPT025Reconciliation-RECOVERY-v1.0.1-PS51.ps1",$CoreFile,$InitFile,$TestFile,$PolicyFile,$SyncDocFile,
        $Sgd000,$Sgd002,$Rmi,$MasterIndex,$MasterRegistry,$Nomenclature,$TraceabilityDoc,
        $MasterInventoryFile,$TraceabilityFile,$DeliveryLedgerFile,$GitLedgerFile,
    ...
    & git.exe commit -m "fix(institutional): recover PS51-safe master synchronization after SPT-025 closure"
