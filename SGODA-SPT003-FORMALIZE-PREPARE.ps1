...
$Version = "1.0.0"
$Self = "SGODA-SPT003-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-003-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt003-real-state-assessment.json"
$CoveragePath = "$Dev/spt003-coverage-matrix.json"
$EvidencePath = "$Dev/spt003-evidence-inventory.json"
$GitPath = "$Dev/spt003-commits-tags-releases.json"
$DependencyPath = "$Dev/spt003-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt003-formalization-prepare.json"
$IntegrityPath = "$Dev/spt003-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"

$DocPath = "docs/06_Tecnologia/SPT-003/SGD-SPT003-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapAssessment = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-002-v1.0.2/spt002-map-reconciliation-assessment.json"
$MapStatus = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-002-v1.0.2/global-deliverable-status-matrix-reconciled.json"
$MapNext = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-002-v1.0.2/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-003 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
try {
    ...
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-003"){
        Hold "Reconciled map does not identify SPT-003 as next audit candidate"
    }

    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-003"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-003 REPOSITORY FOOTPRINT"
    ...
    Write-Host "SPT003_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT003_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT003_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT003_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT003_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT003_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT003_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT003_ACTA_PATHS=$($Actas.Count)"

    if($Direct.Count -lt 1){ Hold "No tracked SPT-003 footprint found" }
    Write-Host "SPT003_DISCOVERY=PASS"
    ...
    $CommitHitsA = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-003" -i)
    $CommitHitsB = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT003" -i)
    ...
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?003|SPT003' })

    Write-Host "SPT003_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT003_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT003_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT003_ACTA_PATHS=$($Actas.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"
    ...
    $Prior = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[1-2]\b' })
    $Later = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[4-6]\b' })

    $CommitAfter003 = @()
    if($CommitSet.Count -gt 0){
        $FirstHash = (($CommitSet[0] -split '\|')[0]).Trim()
        if($FirstHash){
            $CommitAfter003 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20)
        }
    }

    Write-Host "SPT001_TO_002_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT004_TO_006_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT003_COMMIT_SAMPLE=$($CommitAfter003.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-003 REAL STATE"
    ...
    Write-Host "SPT003_REAL_STATE=$RealState"
    Write-Host "SPT003_REASON=$Reason"
    Write-Host "SPT003_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $DependencyObj=[ordered]@{
        component="SPT-003"
        baseline=$ExpectedBaseline
        spt001_to_002_related_paths=$Prior.Count
        spt004_to_006_related_paths=$Later.Count
        post_spt003_commit_sample=@($CommitAfter003)
        sequence_gate="PASS"
    }
    ...
    $PrepareObj=[ordered]@{
        component="SPT-003.FORMALIZE.PREPARE"
        version=$Version
        baseline=$ExpectedBaseline
        real_state=$RealState
    ...
    $Doc=@"
# SPT-003.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado historico real de SPT-003 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    Write-Host "SPT-003.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT003_REAL_STATE=$RealState"
    Write-Host "SPT003_RECOMMENDED_ACTION=$RecommendedAction"
