...
$Version = "1.0.0"
$Self = "SGODA-SPT002-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-002-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt002-real-state-assessment.json"
$CoveragePath = "$Dev/spt002-coverage-matrix.json"
$EvidencePath = "$Dev/spt002-evidence-inventory.json"
$GitPath = "$Dev/spt002-commits-tags-releases.json"
$DependencyPath = "$Dev/spt002-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt002-formalization-prepare.json"
$IntegrityPath = "$Dev/spt002-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"

$DocPath = "docs/06_Tecnologia/SPT-002/SGD-SPT002-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapAssessment = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1/spt02412-map-reconciliation-assessment.json"
$MapStatus = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1/global-deliverable-status-matrix-reconciled.json"
$MapNext = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-02412-v1.0.1/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-002 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
try {
    ...
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-002"){ Hold "Reconciled map does not identify SPT-002 as next audit candidate" }

    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-002"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-002 REPOSITORY FOOTPRINT"
    ...
    Write-Host "SPT002_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT002_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT002_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT002_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT002_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT002_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT002_ACTA_PATHS=$($Actas.Count)"

    if($Direct.Count -lt 1){ Hold "No tracked SPT-002 footprint found" }
    Write-Host "SPT002_DISCOVERY=PASS"
    ...
    foreach($P in $EvidenceCandidates){
        if(Contains-AnyMarker $P @('(?i)\bPASS\b','(?i)FINAL_EXIT_CODE\s*=\s*0','(?i)\bAPPROVED\b','(?i)\bCLOSED\b')){
    ...
    $Prior = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?001\b' })
    $Later = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[3-5]\b' })

    $CommitAfter002 = @()
    if($CommitSet.Count -gt 0){
        $FirstHash = (($CommitSet[0] -split '\|')[0]).Trim()
        if($FirstHash){
            $CommitAfter002 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20)
        }
    }

    Write-Host "SPT001_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT003_TO_005_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT002_COMMIT_SAMPLE=$($CommitAfter002.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-002 REAL STATE"
    ...
    if($HasClosure -and $HasEvidence -and $HasHistory){
        $RealState = "ALREADY_CLOSED"
        $Reason = "Historical closure markers, evidence and commit history are present in tracked repository content."
        $RecommendedAction = "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"
    }
    elseif($HasImplementation -and ($HasEvidence -or $HasHistory)){
    ...
    Write-Host "SPT002_REAL_STATE=$RealState"
    Write-Host "SPT002_REASON=$Reason"
    Write-Host "SPT002_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $DependencyObj=[ordered]@{
        component="SPT-002"
        baseline=$ExpectedBaseline
        spt001_related_paths=$Prior.Count
        spt003_to_005_related_paths=$Later.Count
        post_spt002_commit_sample=@($CommitAfter002)
        sequence_gate="PASS"
    }
    ...
    $PrepareObj=[ordered]@{
        component="SPT-002.FORMALIZE.PREPARE"
        version=$Version
        baseline=$ExpectedBaseline
        real_state=$RealState
    ...
    $Doc=@"
# SPT-002.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado historico real de SPT-002 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    Write-Host "SPT-002.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT002_REAL_STATE=$RealState"
    Write-Host "SPT002_RECOMMENDED_ACTION=$RecommendedAction"
