...
try {
    ...
    Write-Host "UNREPRESENTED_UNTRACKED=$($ResidualUnexpected.Count)"

    if($ResidualUnexpected.Count -ne 0){
        $ResidualUnexpected | ForEach-Object { Write-Host ("UNREPRESENTED="+$_) }
        Hold "Unrepresented untracked content exists after R119"
    }
...
- Contenido institucional unico representado: PASS.
...
- Contenido unico institucional representado: PASS.
- Limpieza destructiva: NO.
- Funcionalidad nueva: NO.
