...
$ExpectedBaseline = "4b9686412d4546a538969929b078acfcdc2f539d"
$Branch = "feature/SPT-001A-rlb-schema-foundation"
$Version = "1.0.4"
$Self = "SGODA-DELIVERABLE-MAP-RECONCILE-004.ps1"

$PreviousMapDir = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-003-v1.0.3"
...
$PreviousAssessment = "$PreviousMapDir/spt003-map-reconciliation-assessment.json"
$PreviousMapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.3.md"

$PrepareScript = "SGODA-SPT004-FORMALIZE-PREPARE.ps1"
$PrepareDir = "artifacts/development/SPT-004-FORMALIZE-PREPARE-v1.0.0"
$PrepareAssessment = "$PrepareDir/spt004-real-state-assessment.json"
$PrepareCoverage = "$PrepareDir/spt004-coverage-matrix.json"
$PrepareEvidence = "$PrepareDir/spt004-evidence-inventory.json"
$PrepareGit = "$PrepareDir/spt004-commits-tags-releases.json"
$PrepareDependency = "$PrepareDir/spt004-dependency-sequence-audit.json"
$PrepareContract = "$PrepareDir/spt004-formalization-prepare.json"
$PrepareIntegrity = "$PrepareDir/spt004-prepare-sha256-manifest.json"
$PrepareImplementationEvidence = "$PrepareDir/implementation-evidence.json"
$PrepareDoc = "docs/06_Tecnologia/SPT-004/SGD-SPT004-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$Dev = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-004-v1.0.4"
...
$ReconciliationAssessment = "$Dev/spt004-map-reconciliation-assessment.json"
...
$MapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.4.md"
$Acta = "docs/00_Estado_Maestro/ACT-SGODA-DELIVERABLE-MAP-RECONCILE-004-v1.0.4.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-004 : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 2 "CONSUME SPT-004 FORMALIZE PREPARE"
    $PrepareFiles = @(
        $PrepareScript,
        $PrepareAssessment,
        $PrepareCoverage,
    ...
    foreach($P in $PrepareFiles){
        if(-not(Test-Path -LiteralPath $P -PathType Leaf)){ Hold ("SPT-004 PREPARE output missing: "+$P) }
    }

    $PrepareState = Read-Json $PrepareAssessment
    $PrepareSpec = Read-Json $PrepareContract

    if([string]$PrepareState.real_state -ne "ALREADY_CLOSED"){ Hold "SPT-004 PREPARE does not certify ALREADY_CLOSED" }
    if([string]$PrepareState.recommended_action -ne "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"){ Hold "Unexpected SPT-004 PREPARE action" }

    Write-Host "SPT004_PREPARE=PASS"
    Write-Host "SPT004_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT004_RECOMMENDED_ACTION=RECONCILE_MASTER_MAP_WITHOUT_REOPENING"
    ...
    if([string]$Next.next_deliverable_candidate -ne "SPT-004"){
        Hold "Previous reconciled map does not identify SPT-004 as current candidate"
    }

    $PendingBefore = @($Status.pending_spt_candidates).Count
    if($PendingBefore -ne 31){ Hold ("Expected 31 pending SPT candidates before SPT-004 reconciliation, found "+$PendingBefore) }
    ...
    Write-Host "PREVIOUS_NEXT_CANDIDATE=SPT-004"
    Write-Host "PREVIOUS_MAP_CONSUMED=PASS"

    Step 4 "RECLASSIFY SPT-004 AS CLOSED WITHOUT REOPENING"
    $Rows = @($Inventory.deliverables)
    $Target = @($Rows | Where-Object { [string]$_.deliverable -eq "SPT-004" })
    if($Target.Count -ne 1){ Hold ("Expected exactly one SPT-004 row, found "+$Target.Count) }

    $PreviousTargetStatus = [string]$Target[0].status
    if($PreviousTargetStatus -ne "IMPLEMENTED_OR_DOCUMENTED"){
        Hold ("Expected SPT-004 classification IMPLEMENTED_OR_DOCUMENTED, found "+$PreviousTargetStatus)
    }
    ...
    foreach($Row in $Rows){
        ...
        if($Id -eq "SPT-004"){
            $ReconciledRows += [ordered]@{
                deliverable = "SPT-004"
                status = "CLOSED"
                reason = "Recertified by SPT-004.FORMALIZE.PREPARE as ALREADY_CLOSED; map corrected without reopening."
    ...
    Write-Host "SPT004_PREVIOUS_CLASSIFICATION=$PreviousTargetStatus"
    Write-Host "SPT004_CLASSIFICATION=CLOSED"
    Write-Host "SPT004_REOPENED=NO"
    ...
    if($PendingIds -contains "SPT-004"){ Hold "SPT-004 remains pending after reconciliation" }

    $ExpectedPendingAfter = $PendingBefore - 1
    if($PendingIds.Count -ne $ExpectedPendingAfter){
        Hold ("Expected "+$ExpectedPendingAfter+" pending SPT candidates after reconciliation, found "+$PendingIds.Count)
    }
    ...
    Write-Host "SPT004_PENDING_AFTER_RECONCILIATION=NO"
    ...
    if($NextCandidate -eq "SPT-004"){ Hold "Next candidate incorrectly remains SPT-004" }
    ...
    $Tags004 = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?004|SPT004' })
    $Tracked = @(& git.exe -c core.quotepath=false ls-files)
    $Release004 = @($Tracked | Where-Object { $_ -match '(?i)^releases/.*SPT[-_.]?004|^releases/.*SPT004' })

    Write-Host "RECENT_COMMITS=$($RecentCommits.Count)"
    Write-Host "SPT004_TAGS=$($Tags004.Count)"
    Write-Host "SPT004_RELEASE_PATHS=$($Release004.Count)"
    ...
    $InventoryObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-004"
        version=$Version
    ...
    $StatusObj=[ordered]@{
        ...
        spt004_classification="CLOSED"
        spt004_reopened=$false
        classification_policy="Repository evidence plus explicit SPT-004 PREPARE recertification."
    }
    ...
    $NextObj=[ordered]@{
        ...
        spt004_removed_from_pending=$true
    }
    ...
    $AssessmentObj=[ordered]@{
        ...
        spt004_real_state="ALREADY_CLOSED"
        spt004_previous_map_status=$PreviousTargetStatus
        spt004_reconciled_status="CLOSED"
        spt004_reopened=$false
    ...
    $LedgerObj=[ordered]@{
        ...
        spt004_prepare_inputs=$PrepareFiles
        ...
        correction=[ordered]@{
            ...
            to="CLOSED"
            reopen=$false
            justification="SPT-004 certified ALREADY_CLOSED by formalization PREPARE."
        }
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
    ...
    $MapDocText=@"
# SGODA-PUINAVE - Mapa Global de Entregables v1.0.4

Linea base: $ExpectedBaseline

## Reconciliacion SPT-004
...
SPT-004 no se reabre. La correccion modifica exclusivamente la clasificacion institucional del mapa y publica la evidencia PREPARE que demuestra su estado real.
"@

    $ActaText=@"
# ACT-SGODA-DELIVERABLE-MAP-RECONCILE-004-v1.0.4

Se certifica la reconciliacion del Mapa Global de Entregables para SPT-004.

- Baseline de entrada: $ExpectedBaseline
- SPT-004 real state: ALREADY_CLOSED
- SPT-004 map classification: CLOSED
- SPT-004 reopened: NO
...
"@
    ...
    Write-Host "MAP_DOCUMENT_V1.0.4=CREATED"
    ...
    Write-Host "SPT004_PREPARE_OUTPUTS_INCLUDED=$($PrepareFiles.Count)"
    ...
    $BlockingUnexpected=@($UnexpectedUntracked | Where-Object {
        $_ -match '(?i)SPT[-_.]?004|SPT004|DELIVERABLE-MAP-RECONCILE-004'
    })
    ...
    Write-Host "BLOCKING_SPT004_UNTRACKED=$($BlockingUnexpected.Count)"

    if($BlockingUnexpected.Count -ne 0){
        $BlockingUnexpected | ForEach-Object { Write-Host ("BLOCKING_UNTRACKED="+$_) }
        Hold "Unexpected active SPT-004 or reconciliation content outside publication set"
    }

    Write-Host "CLOSED_BASELINE_PRESERVED=PASS"
    Write-Host "SPT004_UNTRACKED_ACCOUNTING=PASS"
    ...
    & git.exe commit -m "chore(institutional): reconcile SPT-004 as already closed"
    ...
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-004 : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT004_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT004_CLASSIFICATION=CLOSED"
    Write-Host "SPT004_REOPENED=NO"
