...
$Version = "1.0.0"
$Self = "SGODA-MASTER-CLOSE-v3.ps1"
...
try {
    ...
    $HistoricalCloseScripts = New-Object System.Collections.ArrayList
    ...
    foreach($P in $CurrentUntracked){
        ...
        if($P -match '^SGODA-MASTER-CLOSE(?:-v[0-9]+)?\.ps1$'){
            [void]$HistoricalCloseScripts.Add($P)
            continue
        }
    ...
    Write-Host "MASTER_CLOSE_HISTORICAL_SCRIPTS=$($HistoricalCloseScripts.Count)"
    Write-Host "MASTER_CLOSE_HISTORICAL_POLICY=PRESERVE_AND_PUBLISH_AS_EVIDENCE"
    ...
    $EvidenceObj = [ordered]@{
        ...
        historical_master_close_scripts = @($HistoricalCloseScripts)
        historical_master_close_policy = "PRESERVE_AND_PUBLISH_AS_EVIDENCE"
...
"@
    ...
    foreach($P in $HistoricalCloseScripts){
    ...
    Write-Host "RELEASE_CREATED=NO"
    Write-Host "GITHUB_SIZE_GATE=PASS"
    Write-Host "MASTER_CLOSE_HISTORICAL_SCRIPTS_PUBLISHED=$($HistoricalCloseScripts.Count)"
    Write-Host "MASTER_CLOSE_HISTORICAL_POLICY=PRESERVE_AND_PUBLISH_AS_EVIDENCE"
