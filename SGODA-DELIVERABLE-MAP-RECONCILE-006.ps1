...
$ExpectedBaseline = "ece5ef430c2a249b0094c3a289a08428bad211dd"
$Branch = "feature/SPT-001A-rlb-schema-foundation"
$Version = "1.0.6"
$Self = "SGODA-DELIVERABLE-MAP-RECONCILE-006.ps1"

$PreviousMapDir = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5"
...
$PreviousAssessment = "$PreviousMapDir/spt005-map-reconciliation-assessment.json"
$PreviousMapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.5.md"

$PrepareScript = "SGODA-SPT006-FORMALIZE-PREPARE.ps1"
$PrepareDir = "artifacts/development/SPT-006-FORMALIZE-PREPARE-v1.0.0"
$PrepareAssessment = "$PrepareDir/spt006-real-state-assessment.json"
$PrepareCoverage = "$PrepareDir/spt006-coverage-matrix.json"
$PrepareEvidence = "$PrepareDir/spt006-evidence-inventory.json"
$PrepareGit = "$PrepareDir/spt006-commits-tags-releases.json"
$PrepareDependency = "$PrepareDir/spt006-dependency-sequence-audit.json"
$PrepareContract = "$PrepareDir/spt006-formalization-prepare.json"
$PrepareIntegrity = "$PrepareDir/spt006-prepare-sha256-manifest.json"
$PrepareImplementationEvidence = "$PrepareDir/implementation-evidence.json"
$PrepareDoc = "docs/06_Tecnologia/SPT-006/SGD-SPT006-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$Dev = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6"
...
$ReconciliationAssessment = "$Dev/spt006-map-reconciliation-assessment.json"
...
$MapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.6.md"
$Acta = "docs/00_Estado_Maestro/ACT-SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-006 : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 2 "CONSUME SPT-006 FORMALIZE PREPARE"
    $PrepareFiles = @(
        $PrepareScript,
        $PrepareAssessment,
        $PrepareCoverage,
    ...
    foreach($P in $PrepareFiles){
        if(-not(Test-Path -LiteralPath $P -PathType Leaf)){ Hold ("SPT-006 PREPARE output missing: "+$P) }
    }

    $PrepareState = Read-Json $PrepareAssessment
    $PrepareSpec = Read-Json $PrepareContract

    if([string]$PrepareState.real_state -ne "ALREADY_CLOSED"){ Hold "SPT-006 PREPARE does not certify ALREADY_CLOSED" }
    if([string]$PrepareState.recommended_action -ne "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"){ Hold "Unexpected SPT-006 PREPARE action" }

    Write-Host "SPT006_PREPARE=PASS"
    Write-Host "SPT006_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT006_RECOMMENDED_ACTION=RECONCILE_MASTER_MAP_WITHOUT_REOPENING"
    ...
    if([string]$Next.next_deliverable_candidate -ne "SPT-006"){
        Hold "Previous reconciled map does not identify SPT-006 as current candidate"
    }

    $PendingBefore = @($Status.pending_spt_candidates).Count
    if($PendingBefore -ne 29){ Hold ("Expected 29 pending SPT candidates before SPT-006 reconciliation, found "+$PendingBefore) }

    Write-Host "PREVIOUS_DELIVERABLE_COUNT=$([int]$Inventory.deliverable_count)"
    Write-Host "PREVIOUS_PENDING_SPT_CANDIDATES=$PendingBefore"
    Write-Host "PREVIOUS_NEXT_CANDIDATE=SPT-006"
    Write-Host "PREVIOUS_MAP_CONSUMED=PASS"

    Step 4 "RECLASSIFY SPT-006 AS CLOSED WITHOUT REOPENING"
    $Rows = @($Inventory.deliverables)
    $Target = @($Rows | Where-Object { [string]$_.deliverable -eq "SPT-006" })
    if($Target.Count -ne 1){ Hold ("Expected exactly one SPT-006 row, found "+$Target.Count) }

    $PreviousTargetStatus = [string]$Target[0].status
    if($PreviousTargetStatus -ne "IMPLEMENTED_OR_DOCUMENTED"){
        Hold ("Expected SPT-006 classification IMPLEMENTED_OR_DOCUMENTED, found "+$PreviousTargetStatus)
    }
    ...
    foreach($Row in $Rows){
        ...
        if($Id -eq "SPT-006"){
            $ReconciledRows += [ordered]@{
                deliverable = "SPT-006"
                status = "CLOSED"
                reason = "Recertified by SPT-006.FORMALIZE.PREPARE as ALREADY_CLOSED; map corrected without reopening."
    ...
    Write-Host "SPT006_PREVIOUS_CLASSIFICATION=$PreviousTargetStatus"
    Write-Host "SPT006_CLASSIFICATION=CLOSED"
    Write-Host "SPT006_REOPENED=NO"
    ...
    if($PendingIds -contains "SPT-006"){ Hold "SPT-006 remains pending after reconciliation" }

    $ExpectedPendingAfter = 28
    if($PendingIds.Count -ne $ExpectedPendingAfter){
        Hold ("Expected 28 pending SPT candidates after reconciliation, found "+$PendingIds.Count)
    }
    ...
    Write-Host "SPT006_PENDING_AFTER_RECONCILIATION=NO"
    Write-Host "PENDING_29_TO_28_GATE=PASS"
    ...
    if($NextCandidate -eq "SPT-006"){ Hold "Next candidate incorrectly remains SPT-006" }
    ...
    $Tags006 = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?006|SPT006' })
    $Tracked = @(& git.exe -c core.quotepath=false ls-files)
    $Release006 = @($Tracked | Where-Object { $_ -match '(?i)^releases/.*SPT[-_.]?006|^releases/.*SPT006' })

    Write-Host "RECENT_COMMITS=$($RecentCommits.Count)"
    Write-Host "SPT006_TAGS=$($Tags006.Count)"
    Write-Host "SPT006_RELEASE_PATHS=$($Release006.Count)"
    ...
    $InventoryObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-006"
        version=$Version
    ...
    $StatusObj=[ordered]@{
        ...
        spt006_classification="CLOSED"
        spt006_reopened=$false
        classification_policy="Repository evidence plus explicit SPT-006 PREPARE recertification."
    }
    ...
    $NextObj=[ordered]@{
        ...
        spt006_removed_from_pending=$true
    }
    ...
    $AssessmentObj=[ordered]@{
        ...
        spt006_real_state="ALREADY_CLOSED"
        spt006_previous_map_status=$PreviousTargetStatus
        spt006_reconciled_status="CLOSED"
        spt006_reopened=$false
        master_map_reconciliation="PASS"
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
        pending_29_to_28_gate="PASS"
    ...
    $LedgerObj=[ordered]@{
        ...
        spt006_prepare_inputs=$PrepareFiles
        ...
        correction=[ordered]@{
            ...
            to="CLOSED"
            reopen=$false
            justification="SPT-006 certified ALREADY_CLOSED by formalization PREPARE."
        }
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
    ...
    $EvidenceObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-006"
        authoritative_input_head=$ExpectedBaseline
        spt006_prepare_consumed=$true
        spt006_reopened=$false
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
        expected_pending_after=28
        next_candidate=$NextCandidate
        destructive_cleanup=$false
        production_change=$false
    }
    ...
    $MapDocText=@"
# SGODA-PUINAVE - Mapa Global de Entregables v1.0.6

Linea base: $ExpectedBaseline

## Reconciliacion SPT-006
...
- Reapertura de SPT-006: NO.
- PREPARE consumido: $PrepareAssessment.
- Candidatos pendientes antes: $PendingBefore.
- Candidatos pendientes despues: $($PendingIds.Count).
- Gate 29 a 28: PASS.
...
SPT-006 no se reabre. La correccion modifica exclusivamente la clasificacion institucional del mapa y publica la evidencia PREPARE que demuestra su estado real.
"@

    $ActaText=@"
# ACT-SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6

Se certifica la reconciliacion del Mapa Global de Entregables para SPT-006.

- Baseline de entrada: $ExpectedBaseline
- SPT-006 real state: ALREADY_CLOSED
- SPT-006 map classification: CLOSED
- SPT-006 reopened: NO
- Pending SPT candidates before: $PendingBefore
- Pending SPT candidates after: $($PendingIds.Count)
- Pending 29 to 28 gate: PASS
...
"@
    ...
    Write-Host "MAP_DOCUMENT_V1.0.6=CREATED"
    ...
    Write-Host "SPT006_PREPARE_OUTPUTS_INCLUDED=$($PrepareFiles.Count)"
    ...
    $BlockingUnexpected=@($UnexpectedUntracked | Where-Object {
        $_ -match '(?i)SPT[-_.]?006|SPT006|DELIVERABLE-MAP-RECONCILE-006'
    })
    ...
    Write-Host "BLOCKING_SPT006_UNTRACKED=$($BlockingUnexpected.Count)"

    if($BlockingUnexpected.Count -ne 0){
        $BlockingUnexpected | ForEach-Object { Write-Host ("BLOCKING_UNTRACKED="+$_) }
        Hold "Unexpected active SPT-006 or reconciliation content outside publication set"
    }

    Write-Host "CLOSED_BASELINE_PRESERVED=PASS"
    Write-Host "SPT006_UNTRACKED_ACCOUNTING=PASS"
    ...
    & git.exe commit -m "chore(institutional): reconcile SPT-006 as already closed"
    ...
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-006 : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT006_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT006_CLASSIFICATION=CLOSED"
    Write-Host "SPT006_REOPENED=NO"
