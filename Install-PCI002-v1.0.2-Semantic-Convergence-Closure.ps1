<#
.SYNOPSIS
    Instala y ejecuta PCI-002 v1.0.2 — Institutional Consolidation Engine — Semantic Convergence Closure.
...
$ArtifactDir = Join-Path `
    $ProjectRoot `
    "artifacts\consolidation\PCI-002-v1.0.2"
...
$Architecture = @'
# PCI-002 v1.0.2 — Institutional Consolidation Engine — Semantic Convergence Closure
...
try {
    ...
    if ($Publish) {
        ...
        & $PublisherPath `
            -Publish `
            -CommitMessage "feat(consolidation): close PCI-002 v1.0.2 semantic convergence closure" `
            -EvidenceCommitMessage "chore(consolidation): publish PCI-002 v1.0.2 evidence"
    ...
    Write-Host "PCI-002 v1.0.2 implementado." -ForegroundColor Green
    ...
    Write-Host "Release: releases\PCI-002-v1.0.2" -ForegroundColor Cyan
