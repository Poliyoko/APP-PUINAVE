...
$Version = "1.0.0"
$Self = "SGODA-SPT02412-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-024.12-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt02412-real-state-assessment.json"
$CoveragePath = "$Dev/spt02412-coverage-matrix.json"
$EvidencePath = "$Dev/spt02412-evidence-inventory.json"
$GitPath = "$Dev/spt02412-commits-tags-releases.json"
$PreparePath = "$Dev/spt02412-formalization-prepare.json"
$IntegrityPath = "$Dev/spt02412-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"

$DocPath = "docs/06_Tecnologia/SPT-024/SPT-024.12/SGD-SPT024.12-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-024.12 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
    Write-Host "REASON : $Reason"
    Write-Host "FILES_MODIFIED=NO"
...
try {
    ...
    Step 2 "DISCOVER SPT-024.12 REPOSITORY FOOTPRINT"
    ...
    Write-Host "SPT02412_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT024_RELATED_PATHS=$($Related024.Count)"
    Write-Host "SPT02412_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT02412_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT02412_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT02412_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT02412_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT02412_RELEASE_PATHS=$($Releases.Count)"

    if($Direct.Count -lt 1){ Hold "No tracked SPT-024.12 footprint found" }

    Write-Host "SPT02412_DISCOVERY=PASS"

    Step 3 "CLOSURE / FORMALIZATION MARKER AUDIT"
    $ClosureMarkers = @(
        '(?i)\bCLOSED\b',
    ...
    Step 4 "COMMIT / TAG / RELEASE AUDIT"
    $CommitHits = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-024.12" -i)
    $CommitHitsCompact = @(& git.exe --no-pager log --all --format="%H|%s" --grep="SPT02412" -i)
    $CommitSet = New-Object System.Collections.ArrayList
    foreach($C in @($CommitHits + $CommitHitsCompact)){
        if($C -and -not $CommitSet.Contains($C)){ [void]$CommitSet.Add($C) }
    }

    $Tags = @(& git.exe tag --list)
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?024[-_.]?12|SPT02412' })

    Write-Host "SPT02412_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT02412_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT02412_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"

    Step 5 "TEST / EVIDENCE COVERAGE AUDIT"
    $EvidenceCandidates = @(
        $Artifacts +
        $Releases +
        ($Direct | Where-Object { $_ -match '(?i)evidence|assessment|manifest|ledger|acta|report|resultado|result' })
    ) | Select-Object -Unique

    $PassEvidence = New-Object System.Collections.ArrayList
    foreach($P in $EvidenceCandidates){
        if(Contains-AnyMarker $P @('(?i)\bPASS\b','(?i)FINAL_EXIT_CODE\s*=\s*0','(?i)\bAPPROVED\b')){
    ...
    Step 6 "DEPENDENCY / SEQUENCE AUDIT WITH SPT-024 PROGRAM"
    $Lower024 = @($Related024 | Where-Object { $_ -match '(?i)SPT-024\.(?:[1-9]|1[01])([^0-9]|$)' })
    $Higher024 = @($Related024 | Where-Object { $_ -match '(?i)SPT-024\.(?:1[3-9]|2[0-9])([^0-9]|$)' })
    $Close024 = @($Related024 | Where-Object { $_ -match '(?i)SPT-024.*CLOSE|Cierre.*SPT-024|ACT-.*SPT024' })

    Write-Host "SPT024_PRIOR_COMPONENT_PATHS=$($Lower024.Count)"
    Write-Host "SPT024_LATER_COMPONENT_PATHS=$($Higher024.Count)"
    Write-Host "SPT024_CLOSE_PATHS=$($Close024.Count)"
    Write-Host "DEPENDENCY_AUDIT=PASS"

    Step 7 "DETERMINE REAL STATE"
    $RealState = "INCOMPLETE"
    $Reason = "Tracked footprint exists but closure/formalization evidence is not sufficient."
    $RecommendedAction = "IDENTIFY_AND_COMPLETE_MISSING_ELEMENTS"

    $HasClosure = ($ClosureFiles.Count -gt 0)
    $HasImplementation = ($ImplementationFiles.Count -gt 0 -or $Source.Count -gt 0 -or $Scripts.Count -gt 0)
    $HasEvidence = ($EvidenceCandidates.Count -gt 0)
    $HasCommitHistory = ($CommitSet.Count -gt 0)

    if($HasClosure -and $HasEvidence){
        $RealState = "ALREADY_CLOSED"
        $Reason = "Closure markers and institutional evidence are already present in tracked repository content."
        $RecommendedAction = "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"
    }
    elseif($HasImplementation -and ($HasEvidence -or $HasCommitHistory)){
        $RealState = "IMPLEMENTED_PENDING_FORMALIZATION"
        $Reason = "Implementation/evidence exists but explicit institutional closure evidence is insufficient."
        $RecommendedAction = "BUILD_FORMALIZATION_CLOSURE_PACKAGE"
    }

    Write-Host "SPT02412_REAL_STATE=$RealState"
    Write-Host "SPT02412_REASON=$Reason"
    Write-Host "SPT02412_RECOMMENDED_ACTION=$RecommendedAction"
    Write-Host "REAL_STATE_GATE=PASS"

    Step 8 "BUILD COVERAGE / EVIDENCE MATRICES"
    $CoverageObj = [ordered]@{
    ...
    $PrepareObj = [ordered]@{
        component = "SPT-024.12.FORMALIZE.PREPARE"
        version = $Version
        baseline = $ExpectedBaseline
        real_state = $RealState
    ...
    Write-Utf8NoBomLf $EvidencePath ($EvidenceObj | ConvertTo-Json -Depth 16)
    Write-Utf8NoBomLf $GitPath ($GitObj | ConvertTo-Json -Depth 12)
    ...
    Step 9 "WRITE AUDIT DOCUMENTATION / IMPLEMENTATION EVIDENCE"
    $Doc = @"
# SPT-024.12.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado real de SPT-024.12 antes de cualquier formalizacion, cierre o desarrollo adicional.
...
- No reabrir componentes cerrados.
- No crear tag.
- No crear release.
- No ejecutar git add.
- No ejecutar commit.
...
"@
    ...
    Step 10 "BUILD SHA-256 MANIFEST"
    $Outputs = @(
        $AssessmentPath,
        $CoveragePath,
        $EvidencePath,
    ...
    Write-Utf8NoBomLf $IntegrityPath ($ManifestObj | ConvertTo-Json -Depth 10)
    ...
    Write-Host "SPT-024.12.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT02412_REAL_STATE=$RealState"
    Write-Host "SPT02412_RECOMMENDED_ACTION=$RecommendedAction"
