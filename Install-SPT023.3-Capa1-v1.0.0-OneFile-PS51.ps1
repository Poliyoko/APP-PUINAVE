...
function Stop-Install {
    ...
    if ($null -ne $CreatedFiles) {
        foreach ($Path in @($CreatedFiles.ToArray()) | Sort-Object Length -Descending) {
    ...
    Write-Host " SPT-023.3 CAPA 1 INSTALLATION : HOLD" -ForegroundColor Red
...
try {
    ...
    Write-Host " SGODA-PUINAVE - SPT-023.3 CAPA 1" -ForegroundColor Cyan
    ...
    foreach ($Rel in $Targets) {
        ...
        if (Test-Path -LiteralPath $Full) {
            throw "Target already exists; installation is intentionally non-overwriting: $Rel"
        }
    }
    ...
    Write-Host ""
    Write-Host "[4/10] IMPLEMENTING SPT-023.3 CAPA 1" -ForegroundColor Yellow
...
'@
    ...
    $EvidenceDir = Join-Path $Root ("artifacts\development\SPT-023.3-v1.0.0\runs\" + $RunId)
    ...
    Write-Host " SPT-023.3 CAPA 1 : IMPLEMENTED / QUALITY GATE PASS" -ForegroundColor Green
