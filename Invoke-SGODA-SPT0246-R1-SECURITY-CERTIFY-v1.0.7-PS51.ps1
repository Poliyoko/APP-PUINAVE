...
$SelfName = "Invoke-SGODA-SPT0246-R1-SECURITY-CERTIFY-v1.0.7-PS51.ps1"
...
function IsTx([string]$P) {
    ...
    # Previous failed SPT-024.6 masters are transaction residue, not part of the 56 historical items.
...
try {
    ...
    # PS 5.1-safe recovery detection: no pipeline assignment, no implicit enumeration edge cases.
    ...
    foreach ($target in $recoveryTargets) {
        if (Test-Path -LiteralPath $target) {
            [void]$existing.Add($target)
            Write-Host ("RECOVERY TARGET PRESENT : {0}" -f $target)
        }
    }
    $previousMasters = @()
    try {
        $previousMasters = @(
            Get-ChildItem -LiteralPath $PWD.Path -File -Filter "Invoke-SGODA-SPT0246-*-PS51.ps1" -ErrorAction Stop |
            Where-Object {
                $_.Name -ne $SelfName -and
                $_.Name -match '^Invoke-SGODA-SPT0246-(?:Capa1-FINAL|R1-SECURITY-CERTIFY)-v[0-9]+\.[0-9]+\.[0-9]+-PS51\.ps1$'
            } |
    ...
    foreach($raw in $tracked){
        ...
        $excluded = ($low -match '(^|/)(tests?|docs|releases|artifacts|builder|templates|examples?|\.venv|venv|build|\.dart_tool)(/|$)')
        $isFlutterClient = (
            ($low -match '\.dart$') -or
            ($low -match '(^|/)(android|ios|lib|flutter|mobile|client)(/|$)') -or
            ($low -match '(^|/)pubspec\.ya?ml$')
        )

        if($isFlutterClient -and -not $excluded){
            [void]$fc.Add($rel)
            continue
        }

        if($low -match '^\.github/workflows/.+\.ya?ml$'){
            [void]$ci.Add($rel)
            continue
        }
    ...
    Write-Host "CI/CD YAML CLASSIFIED              : $($ci.Count)"
    Write-Host "REPOSITORY YAML CONFIG CLASSIFIED  : $($cfg.Count)"
    Write-Host "STATIC ANALYSIS ONLY               : YES"
    Write-Host "FLUTTER APP EXECUTED BY GATE       : NO"
    Write-Host "EXTERNAL CONNECTION OPENED         : NO"
    Write-Host "PRODUCTION-SCOPE CLASSIFIER        : PASS"
...
'@
    ...
    # v1.0.7: use the exact same production-scope semantics as Step 4.
    # YAML by itself is NOT a Flutter/mobile production surface.
    $TrackedAll = @(& git.exe -c core.quotepath=false ls-files)
    if ($LASTEXITCODE -ne 0) {
        Stop-Hold "Unable to enumerate Git-tracked files for client assessment."
    }

    $TrackedClientFiles = New-Object System.Collections.ArrayList
    $CiCdFiles = New-Object System.Collections.ArrayList
    $RepositoryConfigFiles = New-Object System.Collections.ArrayList

    foreach ($raw in $TrackedAll) {
        $rel = Norm $raw
        $low = $rel.ToLowerInvariant()
        ...
        if ($isFlutterClient -and -not $excluded) {
            [void]$TrackedClientFiles.Add($rel)
            continue
        }

        if ($low -match '^\.github/workflows/.+\.ya?ml$') {
            [void]$CiCdFiles.Add($rel)
            continue
        }

        if ($low -match '\.ya?ml$') {
            [void]$RepositoryConfigFiles.Add($rel)
        }
    }

    Write-Host "ASSESSMENT MODE          : AUTHORITATIVE GIT-TRACKED CLASSIFIED SCOPE"
    Write-Host "ASSESSMENT EXECUTION     : START"
    Write-Host "FLUTTER CLIENT SURFACES  : $($TrackedClientFiles.Count)"
    Write-Host "CI/CD YAML               : $($CiCdFiles.Count)"
    Write-Host "REPOSITORY YAML CONFIG   : $($RepositoryConfigFiles.Count)"

    foreach($x in $CiCdFiles){
        Write-Host "CLASSIFIED CI/CD         : $x"
    }
    foreach($x in $RepositoryConfigFiles){
        Write-Host "CLASSIFIED REPO CONFIG   : $x"
    }

    # The authoritative baseline currently contains no production Flutter client.
    # Certify that fact explicitly instead of pretending that CI/CD/config YAML is
    # a mobile application surface.
    if ($TrackedClientFiles.Count -eq 0) {
        $ControlIds = @("CLI-SECRETS","CLI-TLS","CLI-CERT","CLI-STORAGE","CLI-LOGGING","CLI-WEBVIEW","CLI-BACKUP")
        $Controls = @()
        foreach ($cid in $ControlIds) {
            $Controls += [ordered]@{
                ...
                blocking = $true
                applicable = $false
                detail = "No production Flutter/client surface exists in the authoritative Git-tracked scope; control is not currently applicable."
            }
        }

        $Classifications = @()
        foreach($x in $CiCdFiles){
            $Classifications += [ordered]@{ path=$x; classification="CI_CD_WORKFLOW"; production_flutter_client=$false }
        }
        foreach($x in $RepositoryConfigFiles){
            $Classifications += [ordered]@{ path=$x; classification="REPOSITORY_CONFIGURATION"; production_flutter_client=$false }
        }

        $Assessment = [ordered]@{
            status = "CLIENT_SECURITY_GATE_PASS"
            scope_result = "NO_PRODUCTION_FLUTTER_CLIENT_PRESENT"
            assessment_mode = "AUTHORITATIVE_GIT_TRACKED_CLASSIFIED_SCOPE"
            production_client_candidates = 0
            ci_cd_yaml_count = $CiCdFiles.Count
            repository_yaml_config_count = $RepositoryConfigFiles.Count
            failed_control_ids = @()
            controls = $Controls
            surfaces = @()
        ...
        # Future-safe branch: if a real Flutter surface exists on the same baseline,
        # assess it with the already tested Python security service rather than
        # silently certifying or misclassifying it.
        $Py = PythonExe
        $env:PYTHONPATH=(Join-Path $PWD "src")

        $ProbePath = Join-Path $env:TEMP ("sgoda-spt0246-r1-"+[Guid]::NewGuid().ToString("N")+".py")
        $ProbeCode = @'
import json
from pathlib import Path
from sgoda.integration.spt0246.service import ClientSecurityService

result = ClientSecurityService(Path.cwd()).assess()
safe = {
    "status": result["status"],
    "scope_result": "PRODUCTION_FLUTTER_CLIENT_ASSESSED",
    "failed_control_ids": result["failed_control_ids"],
...
if result["status"] != "CLIENT_SECURITY_GATE_PASS":
    raise SystemExit(20)
'@
        try {
            $uProbe = New-Object System.Text.UTF8Encoding($false)
            [IO.File]::WriteAllText($ProbePath, (($ProbeCode -replace "`r`n","`n") -replace "`r","`n"), $uProbe)
            $RawAssessment = & $Py $ProbePath
            if ($LASTEXITCODE -ne 0) {
                Stop-Hold "Real production Flutter/client surface failed the blocking security assessment."
            }
            $Assessment = ($RawAssessment -join "`n") | ConvertFrom-Json
        }
        finally {
    ...
    $FailedCount = @($Assessment.failed_control_ids).Count
    Write-Host "CLIENT_SECURITY_STATUS=$($Assessment.status)"
    Write-Host "SCOPE_RESULT=$($Assessment.scope_result)"
    Write-Host "CLIENT_SURFACES=$($TrackedClientFiles.Count)"
    Write-Host "BLOCKING_CONTROLS=7"
    Write-Host "FAILED_BLOCKING_CONTROLS=$FailedCount"
    Write-Host "FAILED_CONTROL_IDS=$(@($Assessment.failed_control_ids) -join ',')"
    Write-Host "EXTERNAL_CONNECTION_OPENED=NO"
    Write-Host "FLUTTER_APP_EXECUTED_BY_GATE=NO"
    Write-Host "SECRET_VALUES_EXPOSED=NO"
    Write-Host "SAFE_ASSESSMENT_REPORT=$AssessmentFile"

    if($Assessment.status -ne "CLIENT_SECURITY_GATE_PASS" -or $FailedCount -ne 0){
        Stop-Hold "SPT-024.6 blocking client security gate did not pass."
    }

    Write-Host "ASSESSMENT EXECUTION  : PASS"
    Write-Host "CLIENT SECURITY GATE  : PASS"
    if($Assessment.scope_result -eq "NO_PRODUCTION_FLUTTER_CLIENT_PRESENT"){
        Write-Host "NO PRODUCTION FLUTTER CLIENT : CERTIFIED"
    }
    ...
    Native "git.exe" @("commit","-m","feat(spt-024.6): certify classified Flutter client security scope") "git commit"
    ...
    Write-Host (" CLIENT_SCOPE_RESULT={0}" -f $Assessment.scope_result) -ForegroundColor Green
