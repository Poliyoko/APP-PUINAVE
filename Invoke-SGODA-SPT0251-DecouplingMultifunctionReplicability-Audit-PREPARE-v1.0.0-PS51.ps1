...
$ArtifactDir="artifacts/development/SPT-025.1-v1.0.0"
...
if($Existing.Count){Write-Host "SPT-025.1 RESUME MODE : ACTIVE"}else{Write-Host "SPT-025.1 FRESH IMPLEMENTATION : ACTIVE"}
...
def classify(path,text=""):
    ...
    if "config" in p or p.endswith((".json",".yaml",".yml",".toml")): reasons.append("CONFIGURABLE_SURFACE")
    if coupling and core: category=CATEGORY_SHARED
...
WriteLf $EvidenceFile ([ordered]@{component="SPT-025.1";version="1.0.0";baseline=$ExpectedBaseline;status="REPLICABILITY_PREPARE_GATE_PASS";targeted_tests="PASS";institutional_suite="PASS";compileall="PASS";production_change=$false;closed_components_preserved=$true}|ConvertTo-Json -Depth 6)
...
Step 14 "COMMIT"
& git.exe commit -m "feat(spt-025.1): audit SGODA decoupling multifunctionality and replicability";if($LASTEXITCODE){Hold "git commit failed"}
