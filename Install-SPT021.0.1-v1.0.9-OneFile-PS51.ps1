...
.SYNOPSIS
    ...
    Version 1.0.8 - One File - Windows PowerShell 5.1
...
.EXAMPLE
    .\Install-SPT021.0.1-v1.0.8-OneFile-PS51.ps1

.EXAMPLE
    .\Install-SPT021.0.1-v1.0.8-OneFile-PS51.ps1 -Publish

.EXAMPLE
    .\Install-SPT021.0.1-v1.0.8-OneFile-PS51.ps1 -Publish -PublishTags
#>
...
function Get-PublicationInventory {
    ...
    # Canonical source for tracked deletions.
    # Git is authoritative; Test-Path alone never declares a deletion.
    $DeletedResult = Invoke-NativeGit -Arguments @(
        "-c",
        "core.quotepath=false",
        "ls-files",
        "--deleted",
    ...
    $DeletedPaths = @(
        $DeletedResult.StdOut -split [char]0 |
        Where-Object { -not [string]::IsNullOrWhiteSpace($_) } |
        ForEach-Object { $_.Replace("\", "/") } |
        Sort-Object -Unique
    )
    ...
    foreach ($DeletedPath in $DeletedPaths) {
        $DeletedLookup[$DeletedPath] = $true
    }
    $UntrackedPaths = @(
    ...
    foreach ($Relative in $CandidatePaths) {
        ...
        if (-not $ExistsAsFile) {
            if ($DeletedLookup.ContainsKey($Relative)) {
...
$RunRoot = Join-Path $ProjectRoot (
    "artifacts\publication\SPT-021.0.1-v1.0.9\runs\" + $RunId
)
$DocsRoot = Join-Path $ProjectRoot "docs\06_Tecnologia\SPT-021.0.1"
$ReleaseRoot = Join-Path $ProjectRoot "releases\SPT-021.0.1-v1.0.9"
...
$PersistentPublisherCopy = Join-Path `
    $ProjectRoot `
    "tools\institutional\Install-SPT021.0.1-v1.0.8-OneFile-PS51.ps1"
...
Write-Step "Validando trazabilidad de version"

$ExpectedRunRootToken = "artifacts\publication\SPT-021.0.1-v1.0.9\runs"
$ExpectedReleaseToken = "releases\SPT-021.0.1-v1.0.9"

$RunRootNormalized = $RunRoot.Replace("/","\")
$ReleaseRootNormalized = $ReleaseRoot.Replace("/","\")

$VersionTraceabilityErrors = 0

if (-not $RunRootNormalized.Contains($ExpectedRunRootToken)) {
    $VersionTraceabilityErrors++
}

if (-not $ReleaseRootNormalized.Contains($ExpectedReleaseToken)) {
    $VersionTraceabilityErrors++
}
...
if ($VersionTraceabilityErrors -ne 0) {
    throw (
        "La trazabilidad interna de version no coincide con " +
        "SPT-021.0.1 v1.0.9."
    )
}
...
Write-Host "Persistent publication engine: SPT-021.0.1 v1.0.9"
