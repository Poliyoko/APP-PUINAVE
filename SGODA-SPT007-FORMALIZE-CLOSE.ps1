...
$Version = "1.0.0"
$Self = "SGODA-SPT007-FORMALIZE-CLOSE.ps1"

$PrepareScript = "SGODA-SPT007-FORMALIZE-PREPARE.ps1"
$PrepareDir = "artifacts/development/SPT-007-FORMALIZE-PREPARE-v1.0.0"
$PrepareAssessment = "$PrepareDir/spt007-real-state-assessment.json"
$PrepareCoverage = "$PrepareDir/spt007-coverage-matrix.json"
$PrepareEvidence = "$PrepareDir/spt007-evidence-inventory.json"
$PrepareGit = "$PrepareDir/spt007-commits-tags-releases.json"
$PrepareDependency = "$PrepareDir/spt007-dependency-sequence-audit.json"
$PrepareContract = "$PrepareDir/spt007-formalization-prepare.json"
$PrepareIntegrity = "$PrepareDir/spt007-prepare-sha256-manifest.json"
$PrepareImplementationEvidence = "$PrepareDir/implementation-evidence.json"
$PrepareDoc = "docs/06_Tecnologia/SPT-007/SGD-SPT007-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$Dev = "artifacts/development/SPT-007-FORMALIZE-CLOSE-v1.0.0"
$CloseAssessment = "$Dev/spt007-formalization-close-assessment.json"
$CoverageRecert = "$Dev/spt007-formalization-close-coverage.json"
$HistoricalTrace = "$Dev/spt007-historical-traceability.json"
$ReleaseRecert = "$Dev/spt007-release-artifact-recertification.json"
$EvidenceRecert = "$Dev/spt007-evidence-recertification.json"
$QualityGate = "$Dev/spt007-formalization-quality-gate.json"
$ImplementationEvidence = "$Dev/implementation-evidence.json"
$CloseIntegrity = "$Dev/spt007-formalization-close-sha256-manifest.json"

$CloseDoc = "docs/06_Tecnologia/SPT-007/SGD-SPT007-FORMALIZATION-CLOSE-v1.0.0.md"
$Acta = "docs/00_Estado_Maestro/ACT-SPT-007-FORMALIZATION-CLOSE-v1.0.0.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-007.FORMALIZE.CLOSE : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 2 "CONSUME SPT-007 FORMALIZE PREPARE"
    $PrepareFiles=@(
        $PrepareScript,
        $PrepareAssessment,
        $PrepareCoverage,
    ...
    foreach($P in $PrepareFiles){
        if(-not(Test-Path -LiteralPath $P -PathType Leaf)){ Hold ("SPT-007 PREPARE output missing: "+$P) }
    }
    ...
    if([string]$Assessment.real_state -ne "IMPLEMENTED_PENDING_FORMALIZATION"){
        Hold "SPT-007 PREPARE does not certify IMPLEMENTED_PENDING_FORMALIZATION"
    }
    if([string]$Assessment.recommended_action -ne "BUILD_FORMALIZATION_CLOSURE_PACKAGE"){
        Hold "SPT-007 PREPARE does not authorize closure package"
    }

    Write-Host "SPT007_PREPARE=PASS"
    Write-Host "SPT007_PREPARE_STATE=IMPLEMENTED_PENDING_FORMALIZATION"
    Write-Host "SPT007_PREPARE_ACTION=BUILD_FORMALIZATION_CLOSURE_PACKAGE"
    ...
    if($TrackedPaths.Count -ne 120){ Hold ("Expected 120 tracked paths from PREPARE, found "+$TrackedPaths.Count) }
    if($Tests.Count -ne 3){ Hold ("Expected 3 historical test paths from PREPARE, found "+$Tests.Count) }
    if($Releases.Count -ne 72){ Hold ("Expected 72 release paths from PREPARE, found "+$Releases.Count) }

    foreach($P in $TrackedPaths){
        $TrackedCheck = @(& git.exe ls-files -- $P)
        if($TrackedCheck.Count -eq 0){ Hold ("PREPARE tracked path no longer tracked: "+$P) }
    }

    Write-Host "SPT007_TRACKED_PATHS_RECERTIFIED=$($TrackedPaths.Count)"
    Write-Host "SPT007_TEST_PATHS_RECERTIFIED=$($Tests.Count)"
    Write-Host "SPT007_DOC_PATHS_RECERTIFIED=$($Docs.Count)"
    Write-Host "SPT007_ARTIFACT_PATHS_RECERTIFIED=$($Artifacts.Count)"
    Write-Host "SPT007_SCRIPT_PATHS_RECERTIFIED=$($Scripts.Count)"
    Write-Host "SPT007_SOURCE_PATHS_RECERTIFIED=$($Source.Count)"
    Write-Host "SPT007_RELEASE_PATHS_RECERTIFIED=$($Releases.Count)"
    Write-Host "SPT007_FOOTPRINT_RECERTIFICATION=PASS"
    ...
    if($EvidenceCandidates.Count -ne 95){ Hold ("Expected 95 evidence candidates from PREPARE, found "+$EvidenceCandidates.Count) }
    if($PassEvidence.Count -ne 9){ Hold ("Expected 9 PASS evidence files from PREPARE, found "+$PassEvidence.Count) }
    if($ImplementationMarkers.Count -ne 25){ Hold ("Expected 25 implementation markers from PREPARE, found "+$ImplementationMarkers.Count) }
    if($ClosureMarkers.Count -ne 0){ Hold "PREPARE unexpectedly contains historical closure markers" }
    ...
    $Actas = @($GitAudit.acta_paths)

    if($Commits.Count -ne 7){ Hold ("Expected 7 SPT-007 commit hits from PREPARE, found "+$Commits.Count) }
    if($ReleasePaths.Count -ne 72){ Hold ("Expected 72 release records from PREPARE, found "+$ReleasePaths.Count) }

    Write-Host "SPT007_COMMIT_HITS_RECERTIFIED=$($Commits.Count)"
    Write-Host "SPT007_TAG_HITS_RECERTIFIED=$($Tags.Count)"
    Write-Host "SPT007_RELEASE_RECORDS_RECERTIFIED=$($ReleasePaths.Count)"
    Write-Host "SPT007_HISTORICAL_ACTAS=$($Actas.Count)"
    Write-Host "TRACEABILITY_RECONSTRUCTION=PASS"

    Step 6 "FORMALIZATION QUALITY GATE"
    $HistoricalTestEvidence = "AVAILABLE"
    $TestRecreation = "NO"
    $TestPolicy = "REUSE_EXISTING_HISTORICAL_TEST_EVIDENCE_ONLY"
    ...
    if($Tests.Count -gt 0 -and $PassEvidence.Count -eq 0){
        Hold "Historical tests exist but no PASS evidence was recertified"
    }
    ...
    $CloseAssessmentObj=[ordered]@{
        component="SPT-007.FORMALIZE.CLOSE"
        version=$Version
    ...
    $QualityObj=[ordered]@{
        ...
        historical_test_evidence=$HistoricalTestEvidence
        historical_tests_recreated=$false
        evidence_candidates=$EvidenceCandidates.Count
        pass_evidence=$PassEvidence.Count
    ...
    $CloseDocText=@"
# SPT-007 - Cierre Institucional Historico v1.0.0
...
## Fundamento
El PREPARE de SPT-007 certifico una implementacion historica real con:
- 120 paths tracked.
- 3 paths de pruebas historicas.
- 95 evidencias candidatas.
- 9 evidencias PASS.
- 25 marcadores de implementacion.
- 72 rutas de release.
- 7 commits relacionados.
- 0 marcadores historicos explicitos de cierre.

Este cierre no reabre ni modifica la implementacion historica. Formaliza institucionalmente el estado ya implementado usando exclusivamente evidencia existente.
...
"@
    ...
    $ActaText=@"
# ACT-SPT-007-FORMALIZATION-CLOSE-v1.0.0

Se formaliza institucionalmente el cierre historico de SPT-007.
...
"@
    ...
    $Blocking=@($UnexpectedUntracked | Where-Object {
        $_ -match '(?i)SPT[-_.]?007|SPT007|FORMALIZE-CLOSE'
    })
    ...
    if($Blocking.Count -ne 0){
        ...
        Hold "Unexpected active SPT-007 closure content outside publication set"
    }
    ...
    Step 14 "COMMIT SPT-007 FORMALIZATION CLOSURE"
    & git.exe commit -m "chore(spt-007): formalize historical institutional closure"
    ...
    Write-Host ""
    Write-Host "SPT-007.FORMALIZE.CLOSE : INSTITUTIONALLY CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT007_PREVIOUS_STATE=IMPLEMENTED_PENDING_FORMALIZATION"
    Write-Host "SPT007_FORMALIZED_STATE=CLOSED"
    Write-Host "SPT007_CLOSURE_TYPE=HISTORICAL_INSTITUTIONAL_FORMALIZATION"
