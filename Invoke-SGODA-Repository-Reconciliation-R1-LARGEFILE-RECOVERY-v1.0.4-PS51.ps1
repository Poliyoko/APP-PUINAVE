...
$SelfName = "Invoke-SGODA-Repository-Reconciliation-R1-LARGEFILE-RECOVERY-v1.0.4-PS51.ps1"
...
try {
    ...
    if($Staged.Count -ne 0){Stop-Hold "Unexpected pre-existing staged changes detected."}
    if($Deleted.Count -ne 0){Stop-Hold "Unexpected tracked deletions detected."}
...
"@
    ...
    foreach($rel in $StagedNow){
        ...
        if(Test-Path -LiteralPath $native -PathType Leaf){
            $len=(Get-Item -LiteralPath $native).Length
            if($len -ge 100MB){[void]$TooLarge.Add([ordered]@{path=$p;bytes=$len})}
        }
    }

    Write-Host "STAGED FILES          : $($StagedNow.Count)"
    Write-Host "FILES >= 100 MB       : $($TooLarge.Count)"

    if($TooLarge.Count -gt 0){
        foreach($x in $TooLarge){Write-Host ("TOO LARGE : {0} ({1} bytes)" -f $x.path,$x.bytes) -ForegroundColor Red}
        Stop-Hold "One or more staged files still exceed GitHub 100 MB limit."
    }

    $TrackedLarge=@(& git.exe ls-files -- $LargeRel)
    if($TrackedLarge.Count -ne 0){Stop-Hold "Oversized monolithic source is still tracked."}

    Write-Host "GITHUB 100 MB FILE GATE : PASS"
