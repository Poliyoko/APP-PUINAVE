...
$Version = "1.0.0"
$Self = "SGODA-SPT006-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-006-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt006-real-state-assessment.json"
$CoveragePath = "$Dev/spt006-coverage-matrix.json"
$EvidencePath = "$Dev/spt006-evidence-inventory.json"
$GitPath = "$Dev/spt006-commits-tags-releases.json"
$DependencyPath = "$Dev/spt006-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt006-formalization-prepare.json"
$IntegrityPath = "$Dev/spt006-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"

$DocPath = "docs/06_Tecnologia/SPT-006/SGD-SPT006-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapAssessment = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5/spt005-map-reconciliation-assessment.json"
$MapStatus = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5/global-deliverable-status-matrix-reconciled.json"
$MapNext = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-006 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
try {
    ...
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-006"){
        Hold "Reconciled map does not identify SPT-006 as next audit candidate"
    }

    $PendingBefore = @($MapStatusObj.pending_spt_candidates).Count
    if($PendingBefore -ne 29){
        Hold ("Expected 29 pending SPT candidates before SPT-006 audit, found "+$PendingBefore)
    }

    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_PENDING_SPT_CANDIDATES=29"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-006"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-006 REPOSITORY FOOTPRINT"
    ...
    Write-Host "SPT006_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT006_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT006_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT006_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT006_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT006_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT006_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT006_ACTA_PATHS=$($Actas.Count)"

    if($Direct.Count -lt 1){ Hold "No tracked SPT-006 footprint found" }
    Write-Host "SPT006_DISCOVERY=PASS"
    ...
    $CommitHitsA = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-006" -i)
    $CommitHitsB = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT006" -i)
    ...
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?006|SPT006' })

    Write-Host "SPT006_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT006_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT006_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT006_ACTA_PATHS=$($Actas.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"
    ...
    $Prior = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[1-5]\b' })
    $Later = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[7-9]\b' })

    $CommitAfter006 = @()
    if($CommitSet.Count -gt 0){
        $FirstHash = (($CommitSet[0] -split '\|')[0]).Trim()
        if($FirstHash){
            $CommitAfter006 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20)
        }
    }

    Write-Host "SPT001_TO_005_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT007_TO_009_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT006_COMMIT_SAMPLE=$($CommitAfter006.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-006 REAL STATE"
    ...
    Write-Host "SPT006_REAL_STATE=$RealState"
    Write-Host "SPT006_REASON=$Reason"
    Write-Host "SPT006_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $DependencyObj=[ordered]@{
        component="SPT-006"
        baseline=$ExpectedBaseline
        spt001_to_005_related_paths=$Prior.Count
        spt007_to_009_related_paths=$Later.Count
        post_spt006_commit_sample=@($CommitAfter006)
        sequence_gate="PASS"
    }
    ...
    $PrepareObj=[ordered]@{
        component="SPT-006.FORMALIZE.PREPARE"
        version=$Version
        baseline=$ExpectedBaseline
        real_state=$RealState
    ...
    $ImplementationObj=[ordered]@{
        ...
        map_candidate="SPT-006"
        map_pending_before=29
        real_state=$RealState
        recommended_action=$RecommendedAction
        git_add=$false
        commit=$false
    ...
    $Doc=@"
# SPT-006.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado historico real de SPT-006 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    Write-Host "SPT-006.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT006_REAL_STATE=$RealState"
    Write-Host "SPT006_RECOMMENDED_ACTION=$RecommendedAction"
