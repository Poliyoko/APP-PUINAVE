<#
.SYNOPSIS
    Aplica SGD-114E v2.0.0-R2.1 — Windows PowerShell Compatible Self Validation Closure.
...
$PmoDir = Join-Path $ProjectRoot "artifacts\pmo\SGD-114E-v2.0.0-R2.1"
$ReportsDir = Join-Path $PmoDir "test-reports"
$ReleaseDir = Join-Path $ProjectRoot "releases\SGD-114E-v2.0.0-R2.1"
$BackupDir = Join-Path `
    $PmoDir `
...
$ComponentPath = Join-Path `
    $ProjectRoot `
    "config\governance\SGD-114E-v2.0.0-R2.1-component.json"

$DocPath = Join-Path `
    $ProjectRoot `
    "docs\01_Gobierno\SGD-114E-v2.0.0-R2.1-WindowsPowerShell-Compatible.md"
...
Get-ChildItem `
    ...
    ForEach-Object {
        ...
        if (
            $null -ne $Object.PSObject.Properties["increment_code"] -and
            -not [string]::IsNullOrWhiteSpace(
                [string]$Object.increment_code
            )
        ) {
            $Code = [string]$Object.increment_code
        }
        elseif (
            $null -ne $Object.PSObject.Properties["component"] -and
            -not [string]::IsNullOrWhiteSpace(
                [string]$Object.component
            )
        ) {
            $Code = [string]$Object.component
        }
        else {
            $Code = [string]$_.BaseName
        }
...
Run "Ejecutando pruebas contractuales SGD-114E" {
    ...
    & $RunnerPath `
        -Component "SGD-114E-v2.0.0-R2.1" `
...
Write-Host "SGD-114E v2.0.0-R2.1 implementado." -ForegroundColor Green
...
Write-Host "Release: releases\SGD-114E-v2.0.0-R2.1.1" -ForegroundColor Cyan
