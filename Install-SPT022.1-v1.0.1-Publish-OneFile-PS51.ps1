<#
.SYNOPSIS
SPT-022.1 v1.0.1 - Publicacion Institucional de SPT-022.
...
$RunRoot = Join-Path `
    $ProjectRoot `
    ("artifacts\publication\SPT-022.1-v1.0.1\runs\" + $RunId)
...
& git fetch origin $Branch --no-tags 2>$null

if ($LASTEXITCODE -ne 0) {
    throw "git fetch de verificacion final fallo."
}
...
Write-Host ""
Write-Host "SPT-022.1 v1.0.1: CLOSED WITH ZERO TECHNICAL ERRORS."
