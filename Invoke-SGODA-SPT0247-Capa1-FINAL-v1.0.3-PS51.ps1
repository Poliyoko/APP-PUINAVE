...
$SelfName = "Invoke-SGODA-SPT0247-Capa1-FINAL-v1.0.3-PS51.ps1"
...
function Git-Fetch-With-Retry {
    ...
    for ($i = 1; $i -le $Attempts; $i++) {
        ...
        # PowerShell 5.1 can promote native STDERR redirected with 2>&1 into a
        # terminating NativeCommandError when $ErrorActionPreference = Stop.
        # Temporarily downgrade only around this read-only network probe so
        # retry logic can actually execute.
    ...
    throw "GitHub connectivity unavailable after $Attempts attempts. Repository was not modified. Last fetch error: $LastMessage"
}
...
try {
    ...
    Write-Host "REMOTE CONNECTIVITY RETRY : ACTIVE (4 ATTEMPTS / PS5.1 SAFE)"
    ...
    Write-Host "v1.0.1 WORKFLOW RUN-LINE FIX   : ACTIVE"
...
'@
    ...
    Native "git.exe" @(
        "commit",
        "-m",
        "feat(spt-024.7): certify CI/CD and supply-chain security layer 1"
    ) "git commit"
