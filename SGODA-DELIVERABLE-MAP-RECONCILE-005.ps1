...
$Version = "1.0.5"
$Self = "SGODA-DELIVERABLE-MAP-RECONCILE-005.ps1"

$PreviousMapDir = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-004-v1.0.4"
...
$PreviousAssessment = "$PreviousMapDir/spt004-map-reconciliation-assessment.json"
$PreviousMapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.4.md"

$PrepareScript = "SGODA-SPT005-FORMALIZE-PREPARE.ps1"
$PrepareDir = "artifacts/development/SPT-005-FORMALIZE-PREPARE-v1.0.0"
$PrepareAssessment = "$PrepareDir/spt005-real-state-assessment.json"
$PrepareCoverage = "$PrepareDir/spt005-coverage-matrix.json"
$PrepareEvidence = "$PrepareDir/spt005-evidence-inventory.json"
$PrepareGit = "$PrepareDir/spt005-commits-tags-releases.json"
$PrepareDependency = "$PrepareDir/spt005-dependency-sequence-audit.json"
$PrepareContract = "$PrepareDir/spt005-formalization-prepare.json"
$PrepareIntegrity = "$PrepareDir/spt005-prepare-sha256-manifest.json"
$PrepareImplementationEvidence = "$PrepareDir/implementation-evidence.json"
$PrepareDoc = "docs/06_Tecnologia/SPT-005/SGD-SPT005-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$Dev = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5"
...
$ReconciliationAssessment = "$Dev/spt005-map-reconciliation-assessment.json"
...
$MapDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Mapa-Global-Entregables-v1.0.5.md"
$Acta = "docs/00_Estado_Maestro/ACT-SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5.md"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-005 : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 2 "CONSUME SPT-005 FORMALIZE PREPARE"
    $PrepareFiles = @(
        $PrepareScript,
        $PrepareAssessment,
        $PrepareCoverage,
    ...
    foreach($P in $PrepareFiles){
        if(-not(Test-Path -LiteralPath $P -PathType Leaf)){ Hold ("SPT-005 PREPARE output missing: "+$P) }
    }

    $PrepareState = Read-Json $PrepareAssessment
    $PrepareSpec = Read-Json $PrepareContract

    if([string]$PrepareState.real_state -ne "ALREADY_CLOSED"){ Hold "SPT-005 PREPARE does not certify ALREADY_CLOSED" }
    if([string]$PrepareState.recommended_action -ne "RECONCILE_MASTER_MAP_WITHOUT_REOPENING"){ Hold "Unexpected SPT-005 PREPARE action" }

    Write-Host "SPT005_PREPARE=PASS"
    Write-Host "SPT005_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT005_RECOMMENDED_ACTION=RECONCILE_MASTER_MAP_WITHOUT_REOPENING"
    ...
    if([string]$Next.next_deliverable_candidate -ne "SPT-005"){
        Hold "Previous reconciled map does not identify SPT-005 as current candidate"
    }

    $PendingBefore = @($Status.pending_spt_candidates).Count
    if($PendingBefore -ne 30){ Hold ("Expected 30 pending SPT candidates before SPT-005 reconciliation, found "+$PendingBefore) }
    ...
    Write-Host "PREVIOUS_NEXT_CANDIDATE=SPT-005"
    Write-Host "PREVIOUS_MAP_CONSUMED=PASS"

    Step 4 "RECLASSIFY SPT-005 AS CLOSED WITHOUT REOPENING"
    $Rows = @($Inventory.deliverables)
    $Target = @($Rows | Where-Object { [string]$_.deliverable -eq "SPT-005" })
    if($Target.Count -ne 1){ Hold ("Expected exactly one SPT-005 row, found "+$Target.Count) }

    $PreviousTargetStatus = [string]$Target[0].status
    if($PreviousTargetStatus -ne "IMPLEMENTED_OR_DOCUMENTED"){
        Hold ("Expected SPT-005 classification IMPLEMENTED_OR_DOCUMENTED, found "+$PreviousTargetStatus)
    }
    ...
    foreach($Row in $Rows){
        ...
        if($Id -eq "SPT-005"){
            $ReconciledRows += [ordered]@{
                deliverable = "SPT-005"
                status = "CLOSED"
                reason = "Recertified by SPT-005.FORMALIZE.PREPARE as ALREADY_CLOSED; map corrected without reopening."
    ...
    Write-Host "SPT005_PREVIOUS_CLASSIFICATION=$PreviousTargetStatus"
    Write-Host "SPT005_CLASSIFICATION=CLOSED"
    Write-Host "SPT005_REOPENED=NO"
    ...
    if($PendingIds -contains "SPT-005"){ Hold "SPT-005 remains pending after reconciliation" }

    $ExpectedPendingAfter = 29
    if($PendingIds.Count -ne $ExpectedPendingAfter){
        Hold ("Expected 29 pending SPT candidates after reconciliation, found "+$PendingIds.Count)
    }
    ...
    Write-Host "SPT005_PENDING_AFTER_RECONCILIATION=NO"
    Write-Host "PENDING_30_TO_29_GATE=PASS"
    ...
    if($NextCandidate -eq "SPT-005"){ Hold "Next candidate incorrectly remains SPT-005" }
    ...
    $Tags005 = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?005|SPT005' })
    $Tracked = @(& git.exe -c core.quotepath=false ls-files)
    $Release005 = @($Tracked | Where-Object { $_ -match '(?i)^releases/.*SPT[-_.]?005|^releases/.*SPT005' })

    Write-Host "RECENT_COMMITS=$($RecentCommits.Count)"
    Write-Host "SPT005_TAGS=$($Tags005.Count)"
    Write-Host "SPT005_RELEASE_PATHS=$($Release005.Count)"
    ...
    $InventoryObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-005"
        version=$Version
    ...
    $StatusObj=[ordered]@{
        ...
        spt005_classification="CLOSED"
        spt005_reopened=$false
        classification_policy="Repository evidence plus explicit SPT-005 PREPARE recertification."
    }
    ...
    $NextObj=[ordered]@{
        ...
        spt005_removed_from_pending=$true
    }
    ...
    $AssessmentObj=[ordered]@{
        ...
        spt005_real_state="ALREADY_CLOSED"
        spt005_previous_map_status=$PreviousTargetStatus
        spt005_reconciled_status="CLOSED"
        spt005_reopened=$false
        master_map_reconciliation="PASS"
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
        pending_30_to_29_gate="PASS"
    ...
    $LedgerObj=[ordered]@{
        ...
        spt005_prepare_inputs=$PrepareFiles
        ...
        correction=[ordered]@{
            ...
            to="CLOSED"
            reopen=$false
            justification="SPT-005 certified ALREADY_CLOSED by formalization PREPARE."
        }
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
    ...
    $EvidenceObj=[ordered]@{
        component="SGODA-DELIVERABLE-MAP-RECONCILE-005"
        authoritative_input_head=$ExpectedBaseline
        spt005_prepare_consumed=$true
        spt005_reopened=$false
        pending_before=$PendingBefore
        pending_after=$PendingIds.Count
        expected_pending_after=29
        next_candidate=$NextCandidate
        destructive_cleanup=$false
        production_change=$false
    }
    ...
    $MapDocText=@"
# SGODA-PUINAVE - Mapa Global de Entregables v1.0.5

Linea base: $ExpectedBaseline

## Reconciliacion SPT-005
...
SPT-005 no se reabre. La correccion modifica exclusivamente la clasificacion institucional del mapa y publica la evidencia PREPARE que demuestra su estado real.
"@

    $ActaText=@"
# ACT-SGODA-DELIVERABLE-MAP-RECONCILE-005-v1.0.5

Se certifica la reconciliacion del Mapa Global de Entregables para SPT-005.

- Baseline de entrada: $ExpectedBaseline
- SPT-005 real state: ALREADY_CLOSED
- SPT-005 map classification: CLOSED
- SPT-005 reopened: NO
- Pending SPT candidates before: $PendingBefore
- Pending SPT candidates after: $($PendingIds.Count)
- Pending 30 to 29 gate: PASS
...
"@
    ...
    Write-Host "MAP_DOCUMENT_V1.0.5=CREATED"
    ...
    Write-Host "SPT005_PREPARE_OUTPUTS_INCLUDED=$($PrepareFiles.Count)"
    ...
    $BlockingUnexpected=@($UnexpectedUntracked | Where-Object {
        $_ -match '(?i)SPT[-_.]?005|SPT005|DELIVERABLE-MAP-RECONCILE-005'
    })
    ...
    Write-Host "BLOCKING_SPT005_UNTRACKED=$($BlockingUnexpected.Count)"

    if($BlockingUnexpected.Count -ne 0){
        $BlockingUnexpected | ForEach-Object { Write-Host ("BLOCKING_UNTRACKED="+$_) }
        Hold "Unexpected active SPT-005 or reconciliation content outside publication set"
    }

    Write-Host "CLOSED_BASELINE_PRESERVED=PASS"
    Write-Host "SPT005_UNTRACKED_ACCOUNTING=PASS"
    ...
    & git.exe commit -m "chore(institutional): reconcile SPT-005 as already closed"
    ...
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-005 : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT005_REAL_STATE=ALREADY_CLOSED"
    Write-Host "SPT005_CLASSIFICATION=CLOSED"
    Write-Host "SPT005_REOPENED=NO"
