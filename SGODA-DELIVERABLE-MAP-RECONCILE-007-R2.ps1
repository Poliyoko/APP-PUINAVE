...
$Self = "SGODA-DELIVERABLE-MAP-RECONCILE-007-R2.ps1"
$R1 = "SGODA-DELIVERABLE-MAP-RECONCILE-007-R1.ps1"
...
function Hold([string]$Reason) {
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-007-R2 : HOLD" -ForegroundColor Red
...
try {
    ...
    if($Deleted.Count -ne 0){
        Hold "Deleted tracked content detected"
    }

    Write-Host "BASELINE_GATE=PASS"
    Write-Host "LOCAL_REMOTE_GATE=PASS"

    Step 2 "RECERTIFY SPT-007 CLOSE INPUTS USING GIT DIFF SEMANTICS"
    ...
    foreach($F in $CloseFiles){
        if(-not(Test-Path -LiteralPath $F -PathType Leaf)){
            Hold "Missing close input: $F"
        }

        $Tracked = @(& git.exe ls-files -- $F)
        if($Tracked.Count -ne 1){
            Hold "Close input is not tracked exactly once: $F"
        }

        & git.exe diff --quiet HEAD -- $F
        ...
        elseif($DiffCode -eq 1){
            ...
            Hold "git diff failed while recertifying close input: $F"
        }
    }

    Write-Host "SPT007_CLOSE_INPUTS=$($CloseFiles.Count)"
    Write-Host "SPT007_CLOSE_INPUTS_TRACKED_UNCHANGED=$TrackedClose"
    Write-Host "SPT007_CLOSE_INPUTS_CHANGED=$($ChangedClose.Count)"

    if($ChangedClose.Count -ne 0){
        foreach($F in $ChangedClose){
            Write-Host "CHANGED_CLOSE_INPUT=$F"
        }
        Hold "Tracked SPT-007 close input has a real Git-visible change"
    }

    Write-Host "SPT007_CLOSE_INPUTS_STAGE_REQUIRED=NO"
    Write-Host "GIT_DIFF_RECERTIFICATION=PASS"

    Step 3 "RECERTIFY FAILED-RUN TRANSACTION STAGING"

    foreach($F in $TransactionOutputs){
        if(-not(Test-Path -LiteralPath $F -PathType Leaf)){
            Hold "Missing transaction output from failed run: $F"
        }
    }

    $CurrentStaged = @(& git.exe -c core.quotepath=false diff --cached --name-only)
    $UnexpectedStaged = @($CurrentStaged | Where-Object {
        $TransactionOutputs -notcontains ($_ -replace "\\","/")
    })
    $MissingStaged = @($TransactionOutputs | Where-Object {
        $CurrentStaged -notcontains $_
    })
    ...
    if($UnexpectedStaged.Count -ne 0){
        $UnexpectedStaged | ForEach-Object { Write-Host "UNEXPECTED_STAGED_PATH=$_" }
        Hold "Unexpected staged content exists"
    }
    if($MissingStaged.Count -ne 0){
        $MissingStaged | ForEach-Object { Write-Host "MISSING_STAGED_PATH=$_" }
        Hold "Failed-run transaction staged set is incomplete"
    }
    if($CurrentStaged.Count -ne $TransactionOutputs.Count){
        Hold "Failed-run transaction staged set count mismatch"
    }

    Write-Host "FAILED_RUN_STAGE_RECERTIFICATION=PASS"

    Step 4 "PRESERVE RECOVERY HISTORY / CORRECT PUBLICATION MODEL"

    foreach($F in @($R1,$Self)){
        if(-not(Test-Path -LiteralPath $F -PathType Leaf)){
            Hold "Recovery script missing: $F"
        }
    }

    $PublicationSet = @($TransactionOutputs + $R1 + $Self)

    foreach($F in @($R1,$Self)){
        & git.exe -c core.autocrlf=false -c core.safecrlf=true add -- $F
        if($LASTEXITCODE -ne 0){
            Hold "git add failed for recovery evidence: $F"
        }
    }

    $StagedNow = @(& git.exe -c core.quotepath=false diff --cached --name-only)
    $UnexpectedNow = @($StagedNow | Where-Object {
        $PublicationSet -notcontains ($_ -replace "\\","/")
    })
    $MissingNow = @($PublicationSet | Where-Object {
        $StagedNow -notcontains $_
    })

    Write-Host "PUBLICATION_SET=$($PublicationSet.Count)"
    Write-Host "TRANSACTION_OUTPUTS=$($TransactionOutputs.Count)"
    Write-Host "RECOVERY_SCRIPTS_PUBLISHED=2"
    Write-Host "TRACKED_IMMUTABLE_CLOSE_INPUTS=$($CloseFiles.Count)"
    Write-Host "STAGED_PUBLICATION_SET=$($StagedNow.Count)"
    ...
    Step 5 "STAGED DELETION / GITHUB SIZE GATE"
    ...
    if($LocalPre -ne $ExpectedBaseline -or $RemotePre -ne $ExpectedBaseline){
        Hold "Remote changed before commit"
    }
    ...
    if($FinalLocal -ne $FinalRemote -or
       [int]$FinalAB[0] -ne 0 -or
       [int]$FinalAB[1] -ne 0 -or
    ...
    Write-Host ""
    Write-Host "SGODA-DELIVERABLE-MAP-RECONCILE-007-R2 : CLOSED / PASS" -ForegroundColor Green
