...
$SelfName = "Invoke-SGODA-SPT0245-R1-SECURITY-CERTIFY-v1.0.7-PS51.ps1"
...
$ArtifactDir = "artifacts/development/SPT-024.5-R1-v1.0.7"
...
function Stop-Hold {
    ...
    Write-Host " SPT-024.5 R1 v1.0.7 : HOLD" -ForegroundColor Red
...
'@
    ...
    $Evidence = [ordered]@{
        ...
        recovery = [ordered]@{
            ...
            v107_fix = "transaction-local safecrlf override for controlled Git staging; canonical LF preserved without changing repository/global Git configuration"
    ...
    $Utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    # v1.0.7: repository Git policy rejects CRLF for this JSON path.
    ...
    # v1.0.7: canonicalize text files created/modified by THIS transaction only.
    ...
    foreach ($target in $StageTargets) {
        if (Test-Path -LiteralPath $target) {
            Invoke-Native -FilePath "git.exe" -ArgumentList @("-c","core.safecrlf=false","add","--",$target) -Label ("git add (transaction-local safecrlf override) " + $target)
        }
    }
