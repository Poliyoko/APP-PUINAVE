...
$R118Dev = "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.8"
...
$R118Act = "docs/00_Estado_Maestro/ACT-SGODA-MASTER-SYNC-TRANSACTION-RECOVERY-v1.1.8.md"
...
function Hold { param([string]$Reason) Write-Host ""; Write-Host "SGODA R118 TRANSACTION RECOVERY : HOLD" -ForegroundColor Red; Write-Host "REASON : $Reason"; Write-Host "TRANSACTION : NOT PUBLISHED"; exit 1 }
...
try {
    ...
    if($Staged.Count -ne 0 -or $Deleted.Count -ne 0){ Hold "Index is not clean" }
    ...
    if(@(& git.exe diff --name-only -- $RuntimeState).Count -eq 1 -and -not $StageBase.Contains($RuntimeState)){ [void]$StageBase.Add($RuntimeState) }
    ...
    Step 6 "CANONICAL PYTHON CONTEXT / FULL RECERTIFICATION"
    ...
    if($LASTEXITCODE -ne 0){ $CollectOutput|ForEach-Object{Write-Host $_}; Hold "Institutional test collection failed" }
    $CollectText=($CollectOutput -join "`n")
    $CollectMatch=[regex]::Match($CollectText,'(?m)(\d+)\s+tests?\s+collected')
    $Collected=if($CollectMatch.Success){[int]$CollectMatch.Groups[1].Value}else{0}
    if($Collected -ne 2409){ Hold ("Unexpected collection count: "+$Collected) }
    Write-Host "INSTITUTIONAL_TEST_COLLECTION=PASS"
    Write-Host "INSTITUTIONAL_TESTS_COLLECTED=$Collected"

    $SuiteOutput=@(& $Python -m pytest tests -q 2>&1)
    $SuiteExit=$LASTEXITCODE
    $SuiteOutput|ForEach-Object{Write-Host $_}
    if($SuiteExit -ne 0){ Hold "Institutional suite failed" }
    $SuiteText=($SuiteOutput -join "`n")
    $PassMatch=[regex]::Match($SuiteText,'(?m)(\d+)\s+passed')
    $Passed=if($PassMatch.Success){[int]$PassMatch.Groups[1].Value}else{0}
    if($Passed -ne 2409){ Hold ("Unexpected passed count: "+$Passed) }
    ...
    Step 7 "R118 SECURITY DELTA GATE"
    ...
    if(Test-NewTextSecret $Self){ Hold "Secret pattern found in R118 script" }
    Write-Host "R117_SECURITY_EVIDENCE_REUSED=PASS"
    Write-Host "R118_DELTA_SECRET_HITS=0"
    Write-Host "SECRET_VALUES_PRINTED=NO"
    Write-Host "SECURITY_GATE=PASS"

    Step 8 "WRITE R118 TRANSACTION EVIDENCE"
    ...
    $EvidenceObj=[ordered]@{
        ...
        canonical_runtime=".venv/Scripts/python.exe + PYTHONPATH=src";test_collection=2409;test_passed=2409;
        staged_before_recovery=0;deleted_tracked_before_recovery=0;commit_performed_at_evidence_write=$false;push_performed_at_evidence_write=$false
    }
    $Doc=@"
...
"@
    $Act=@"
# ACT-SGODA-MASTER-SYNC-TRANSACTION-RECOVERY-v1.1.8
...
"@
    ...
    Write-Host "R118_EVIDENCE=CREATED"
    Write-Host "R118_ACTA=CREATED"
    ...
    $StageManifestObj=[ordered]@{
        authoritative_input_head=$ExpectedBaseline;stage_count=$StageSet.Count;raw_oversized_excluded=@($RawOversized);
        residual_exact_duplicates_not_staged=@($ResidualDuplicate);residual_superseded_outputs_not_staged=@($ResidualSuperseded);paths=@($StageSet)
    }
    ...
    $UnexpectedStaged=@($StagedNow|Where-Object{$StageSet -notcontains ($_ -replace "\\","/")})
    ...
    if($UnexpectedStaged.Count -ne 0 -or $StagedNow.Count -ne $StageSet.Count){ Hold "Exact staging mismatch" }
    ...
    Write-Host "R118_STAGED_DELTA_SECRET_HITS=0"
    ...
    & git.exe commit -m "fix(institutional): complete R118 master synchronization transaction recovery"
    ...
    Write-Host ""
    Write-Host "SGODA R118 TRANSACTION RECOVERY : CLOSED / PASS" -ForegroundColor Green
    Write-Host "R118_TRANSACTION_RECOVERY=PASS"
