...
function Fail {
    ...
    Write-Host " SGODA RECONCILIATION v2.0.2 : HOLD" -ForegroundColor Red
...
function Write-Utf8Lf {
    ...
    $Text = ($Lines -join "`n").TrimEnd([char[]]@("`r","`n")) + "`n"
...
if ([string]::IsNullOrWhiteSpace($ProjectRoot)) {
    $ProjectRoot = ([string](Get-GitOutput @("rev-parse","--show-toplevel") | Select-Object -First 1)).Trim()
}

Set-Location $ProjectRoot

$Root = ([string](Get-GitOutput @("rev-parse","--show-toplevel") | Select-Object -First 1)).Trim()
Set-Location $Root

$Branch = ([string](Get-GitOutput @("branch","--show-current") | Select-Object -First 1)).Trim()
$Origin = ([string](Get-GitOutput @("remote","get-url","origin") | Select-Object -First 1)).Trim()

# PowerShell 5.1 compatibility self-test: prove scalar Git output is handled as string, not Char.
$SelfTestHead = [string](Get-GitOutput @("rev-parse","HEAD") | Select-Object -First 1)
if ([string]::IsNullOrWhiteSpace($SelfTestHead) -or $SelfTestHead.Length -lt 7) {
    Fail "PowerShell 5.1 Git scalar-output self-test failed."
}

Write-Section "SGODA-PUINAVE - TRANSACTIONAL UNTRACKED RECONCILIATION v2.0.2"
...
$LocalHead = ([string](Get-GitOutput @("rev-parse","HEAD") | Select-Object -First 1)).Trim()
$RemoteHead = ([string](Get-GitOutput @("rev-parse","origin/$Branch") | Select-Object -First 1)).Trim()
...
if ($LocalHead -ne $ExpectedHead) {
    ...
    try {
        $Parent = ([string](Get-GitOutput @("rev-parse","HEAD^") | Select-Object -First 1)).Trim()
        $Subject = ([string](Get-GitOutput @("log","-1","--pretty=%s") | Select-Object -First 1)).Trim()
    }
    catch {
    ...
    if (
        ...
        $RemoteAfterResume = ([string](Get-GitOutput @("rev-parse","origin/$Branch") | Select-Object -First 1)).Trim()
...
$InstitutionalTool = Join-Path `
    $Root `
    "tools\institutional\Invoke-SGODA-Untracked-Reconciliation-v2.0.2-PS51.ps1"
...
$EvidenceRoot = Join-Path `
    $Root `
    "artifacts\audit\untracked-reconciliation-v2.0.2"
...
foreach ($Name in $AuditEvidenceFiles) {
    ...
    if (Test-Path -LiteralPath $Full -PathType Leaf) {
        $Relative = Normalize-Slashes ($Full.Substring($Root.Length).TrimStart([char[]]@("\","/")))
...
$ToolRelative = Normalize-Slashes ($InstitutionalTool.Substring($Root.Length).TrimStart([char[]]@("\","/")))
$EvidenceRelative = Normalize-Slashes ($EvidencePath.Substring($Root.Length).TrimStart([char[]]@("\","/")))
...
# ----------------------------------------------------------------------
# Python environment preflight BEFORE any staging
# ----------------------------------------------------------------------

Write-Section "6/11 - PYTHON ENVIRONMENT PREFLIGHT"

$VenvPython = Join-Path $Root ".venv\Scripts\python.exe"

if (-not (Test-Path -LiteralPath $VenvPython -PathType Leaf)) {
    Fail "Project virtual environment Python not found: $VenvPython"
}
...
try {
    ...
    $PytestVersion = @(& $PythonExe -m pytest --version 2>&1)
    $PytestVersionCode = $LASTEXITCODE

    $FastApiCheck = @(& $PythonExe -c "import fastapi; print(fastapi.__version__)" 2>&1)
    $FastApiCode = $LASTEXITCODE
}
finally {
    $ErrorActionPreference = $PreviousEap
}

Write-Host "PYTHON VERSION : $($PythonVersion -join ' ')"
Write-Host "PYTEST VERSION : $($PytestVersion -join ' ')"
Write-Host "FASTAPI CHECK  : $($FastApiCheck -join ' ')"

if ($PythonVersionCode -ne 0) {
    Fail "Project .venv Python is not executable."
}

if ($PytestVersionCode -ne 0) {
    Fail "pytest is not installed in the project .venv."
}

if ($FastApiCode -ne 0) {
    Fail "fastapi is not installed in the project .venv."
}
...
Write-Section "7/11 - TRANSACTIONAL CONTROLLED STAGING"
...
Write-Section "8/11 - ABSOLUTE EXCLUSION AND GIT QUALITY GATE"
...
Write-Section "9/11 - PROJECT QUALITY GATES"
...
$PytestText = ($PytestOutput | ForEach-Object { [string]$_ }) -join "`n"
...
Write-Host "PYTHON COMPILEALL : PASS" -ForegroundColor Green
    ...
    & git reset -q HEAD -- 2>$null
    Fail ("Project quality gate raised an exception; index rolled back: " + $_.Exception.Message)
}
...
Write-Section "10/11 - FINAL PUBLICATION GATE"
...
$HeadBeforeCommit = ([string](Get-GitOutput @("rev-parse","HEAD") | Select-Object -First 1)).Trim()
$RemoteBeforeCommit = ([string](Get-GitOutput @("rev-parse","origin/$Branch") | Select-Object -First 1)).Trim()
...
Write-Section "11/11 - COMMIT, PUSH, AND REMOTE VERIFICATION"
...
$PublishedCommit = ([string](Get-GitOutput @("rev-parse","HEAD") | Select-Object -First 1)).Trim()
...
$LocalFinal = ([string](Get-GitOutput @("rev-parse","HEAD") | Select-Object -First 1)).Trim()
$RemoteFinal = ([string](Get-GitOutput @("rev-parse","origin/$Branch") | Select-Object -First 1)).Trim()
...
Write-Host " SGODA RECONCILIATION v2.0.2 : PASS" -ForegroundColor Green
