...
$SelfName = "Invoke-SGODA-Repository-Reconciliation-FINAL-v1.0.0-PS51.ps1"
...
function Test-SafeTextCandidate {
    ...
    if(-not(Test-Path -LiteralPath $native -PathType Leaf)){
        return [pscustomobject]@{safe=$false;reason="FILE_NOT_FOUND";secret_markers=0}
    }
    ...
    if($textExtensions -notcontains $ext){
        return [pscustomobject]@{safe=$true;reason="BINARY_OR_NON_TEXT_NO_CONTENT_SCAN";secret_markers=0}
    }

    try{$text=[IO.File]::ReadAllText((Resolve-Path -LiteralPath $native).Path,[Text.Encoding]::UTF8)}
    catch{return [pscustomobject]@{safe=$false;reason="TEXT_READ_FAILED";secret_markers=0}}

    $patterns=@(
        '(?im)^\s*(api[_-]?key|secret|token|password|passwd|client[_-]?secret)\s*[:=]\s*["''][^"'']{8,}["'']\s*$',
    ...
    $hits=0
    foreach($pattern in $patterns){$hits += ([regex]::Matches($text,$pattern)).Count}
    if($hits -gt 0){
        return [pscustomobject]@{safe=$false;reason="SECRET_LIKE_CONTENT_REQUIRES_REVIEW";secret_markers=$hits}
    }
    return [pscustomobject]@{safe=$true;reason="SAFE_STATIC_CONTENT_SCAN";secret_markers=0}
}
...
try {
    ...
    foreach($p in $CandidatePaths){
        ...
        if(-not $scan.safe){
            [void]$BlockedCandidates.Add([ordered]@{path=$p;reason=$scan.reason;secret_markers=$scan.secret_markers})
            continue
        }
    ...
    Write-Host "BLOCKED SENSITIVE REVIEW : $($BlockedCandidates.Count)"
    Write-Host "SECRET VALUES PRINTED    : NO"

    if($BlockedCandidates.Count -gt 0){
        New-Item -ItemType Directory -Force -Path $ArtifactDir|Out-Null
        $PreReport=[ordered]@{
            status="RECONCILIATION_HOLD";baseline=$ExpectedBaseline;generated_utc=[DateTime]::UtcNow.ToString("o");
            historical_items=$Historical.Count;candidate_count=$CandidatePaths.Count;blocked_sensitive_count=$BlockedCandidates.Count;
            blocked_sensitive=@($BlockedCandidates);secret_values_exposed=$false
        }
        Write-Lf $InventoryFile ($PreReport|ConvertTo-Json -Depth 10)
        Stop-Hold "One or more publication candidates contain secret-like content and require safe review. Values were not exposed."
    }
    ...
    $Inventory=[ordered]@{
        ...
        blocked_sensitive_items=0;classifications=@($Classifications);publish_candidates=@($SafeCandidates);
...
"@
    ...
    $Evidence=[ordered]@{
        ...
        gates=[ordered]@{remote_baseline="PASS";sha256_inventory="PASS";duplicate_detection="PASS";sensitive_content="PASS";
