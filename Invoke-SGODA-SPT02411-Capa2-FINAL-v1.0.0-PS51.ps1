...
$SelfName="Invoke-SGODA-SPT02411-Capa2-FINAL-v1.0.0-PS51.ps1"
...
function Native([string]$Exe,[string[]]$Args,[string]$Label){ & $Exe @Args; if($LASTEXITCODE -ne 0){ throw "$Label failed with exit code $LASTEXITCODE" } }
function WriteLf([string]$Path,[string]$Text){
  ...
  $Canonical=(($Text -replace "`r`n","`n") -replace "`r","`n"); if(-not $Canonical.EndsWith("`n")){$Canonical+="`n"}
...
Write-Host "DATA LIFECYCLE SURFACES : $($Surfaces.Count)"; Write-Host "DISCOVERY MODE          : STATIC / NON-DESTRUCTIVE"; Write-Host "PRODUCTION DATA DELETED : NO"
...
root=Path.cwd(); paths=sys.argv[1:]
r=DataLifecycleGovernanceService(root,paths).assess()
ad=root/"artifacts/development/SPT-024.11-Capa2-v1.0.0"; ad.mkdir(parents=True,exist_ok=True)
files={
...
for name,payload in files.items():(ad/name).write_text(json.dumps(payload,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
manifest=build_manifest(root,[str((ad/n).relative_to(root)).replace("\\","/") for n in files]+["config/integration/spt02411/data-lifecycle-governance-policy.json"])
(ad/"data-lifecycle-integrity-manifest.json").write_text(json.dumps(manifest,indent=2)+"\n",encoding="utf-8")
...
print("PRODUCTION_DATA_MODIFIED=NO");print("PRODUCTION_DATA_DELETED=NO");print("ARCHIVE_EXECUTED=NO");print("DISPOSAL_EXECUTED=NO");print("SECRET_VALUES_EXPOSED=NO")
raise SystemExit(0 if r["status"]=="DATA_LIFECYCLE_GOVERNANCE_GATE_PASS" else 20)
'@
WriteLf $Tmp $Probe
& $Python $Tmp @($Surfaces|ForEach-Object{$_ -replace '\\','/'}); $ec=$LASTEXITCODE
