...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ScriptPath)) {
        throw "Unable to resolve current master script path."
    }

    $ScriptPath = (Resolve-Path -LiteralPath $ScriptPath).Path
    if (-not $ScriptPath.StartsWith($Root, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Master script must be executed from inside the official repository."
    }

    $ScriptRel = $ScriptPath.Substring($Root.Length).TrimStart([char[]]@("\","/")).Replace("\","/")
