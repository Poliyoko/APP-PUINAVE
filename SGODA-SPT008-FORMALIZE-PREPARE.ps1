...
$Version = "1.0.0"
$Self = "SGODA-SPT008-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-008-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt008-real-state-assessment.json"
$CoveragePath = "$Dev/spt008-coverage-matrix.json"
$EvidencePath = "$Dev/spt008-evidence-inventory.json"
$GitPath = "$Dev/spt008-commits-tags-releases.json"
$DependencyPath = "$Dev/spt008-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt008-formalization-prepare.json"
$IntegrityPath = "$Dev/spt008-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"
$DocPath = "docs/06_Tecnologia/SPT-008/SGD-SPT008-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapDir = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-007-v1.0.7"
$MapAssessment = "$MapDir/spt007-map-reconciliation-assessment.json"
$MapStatus = "$MapDir/global-deliverable-status-matrix-reconciled.json"
$MapNext = "$MapDir/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-008 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
try {
    ...
    foreach($P in @($MapAssessment,$MapStatus,$MapNext)){
        if(-not(Test-Path -LiteralPath $P -PathType Leaf)){
            Hold ("Reconciled map input missing: "+$P)
        }
    }
    ...
    if($PendingBefore -ne 27){
        Hold ("Expected 27 pending SPT candidates before SPT-008 audit, found "+$PendingBefore)
    }
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-008"){
        Hold "Reconciled map does not identify SPT-008 as next audit candidate"
    }

    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_PENDING_SPT_CANDIDATES=27"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-008"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-008 REPOSITORY FOOTPRINT"
    ...
    $ProgramRelated = Find-Tracked @(
        '(?i)(^|/)SPT-0(0[1-9]|1[0-9])([^0-9]|$)',
        '(?i)\bSPT[-_.]?0(0[1-9]|1[0-9])\b'
    )
    ...
    Write-Host "SPT008_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT008_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT008_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT008_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT008_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT008_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT008_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT008_ACTA_PATHS=$($Actas.Count)"

    if($Direct.Count -lt 1){
        Hold "No tracked SPT-008 footprint found"
    }

    Write-Host "SPT008_DISCOVERY=PASS"
    ...
    $CommitHitsA = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-008" -i)
    $CommitHitsB = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT008" -i)
    ...
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?008|SPT008' })

    Write-Host "SPT008_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT008_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT008_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT008_ACTA_PATHS=$($Actas.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"
    ...
    $EvidenceCandidates = @(
        ...
        $Actas +
        ($Direct | Where-Object {
            $_ -match '(?i)evidence|assessment|manifest|ledger|acta|report|resultado|result|audit|certif'
        })
    ...
    $Prior = @($ProgramRelated | Where-Object {
        $_ -match '(?i)SPT[-_.]?00[1-7]\b'
    })

    $Later = @($ProgramRelated | Where-Object {
        $_ -match '(?i)SPT[-_.]?0(0[9]|1[0-9])\b'
    })
    ...
    if($CommitSet.Count -gt 0){
        ...
        if($FirstHash){
            $CommitAfter008 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20)
        }
    }

    Write-Host "SPT001_TO_007_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT009_TO_019_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT008_COMMIT_SAMPLE=$($CommitAfter008.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-008 REAL STATE"
    ...
    Write-Host "SPT008_REAL_STATE=$RealState"
    Write-Host "SPT008_REASON=$Reason"
    Write-Host "SPT008_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $DependencyObj=[ordered]@{
        component="SPT-008"
        baseline=$ExpectedBaseline
        spt001_to_007_related_paths=$Prior.Count
        spt009_to_019_related_paths=$Later.Count
        post_spt008_commit_sample=@($CommitAfter008)
        sequence_gate="PASS"
    }
    ...
    $PrepareObj=[ordered]@{
        component="SPT-008.FORMALIZE.PREPARE"
        version=$Version
        baseline=$ExpectedBaseline
        real_state=$RealState
    ...
    $ImplementationObj=[ordered]@{
        ...
        map_candidate="SPT-008"
        map_pending_before=27
        real_state=$RealState
        recommended_action=$RecommendedAction
        git_add=$false
        commit=$false
    ...
    $Doc=@"
# SPT-008.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado historico real de SPT-008 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    Write-Host "SPT-008.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT008_REAL_STATE=$RealState"
    Write-Host "SPT008_RECOMMENDED_ACTION=$RecommendedAction"
