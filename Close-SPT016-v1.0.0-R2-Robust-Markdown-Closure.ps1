<#
.SYNOPSIS
    Ejecuta SPT-016 v1.0.0-R2 — Robust Markdown Official Closure.
...
$SpecificResultText = "NO APROBADO"

if ([bool]$Specific.approved) {
    $SpecificResultText = "APROBADO"
}
...
$TestingLines = @(
    ...
    "## Pruebas específicas",
    "",
    ("- Ejecutadas: " + [string]$Specific.executed),
    ("- Aprobadas: " + [string]$Specific.passed),
    ("- Fallidas: " + [string]$Specific.failures),
    ("- Errores: " + [string]$Specific.errors),
    ("- Omitidas: " + [string]$Specific.skipped),
    ("- Resultado: " + $SpecificResultText),
    "",
    "## Evidencia",
    "",
...
$TestingContent = [string]::Join(
    [Environment]::NewLine,
    $TestingLines
)

Write-Utf8 -Path $TestingDoc -Content $TestingContent
...
$ClosureLines = @(
    ...
    "## Evidencias",
    "",
    ("- Archivos fuente inventariados: " + [string]$SourceInventory.Count),
    (
        "- Pruebas específicas: " +
...
$ClosureContent = [string]::Join(
    [Environment]::NewLine,
    $ClosureLines
)

Write-Utf8 -Path $ClosureMd -Content $ClosureContent
Write-Utf8 -Path $ClosureAct -Content $ClosureContent
...
Write-Host "SPT-016 v1.0.0-R2 cerrado oficialmente." -ForegroundColor Green
