<#
.SYNOPSIS
    Instala SPT-019.1 v1.0.1 — Workflow Engine Core.

.DESCRIPTION
    Implementa el núcleo Python, workflows iniciales, pruebas, registro
    determinístico, simulación controlada basada en archivo, evidencias y
    release de integración. No instala ni inicia n8n.
#>
...
$SourceDir = Join-Path $ProjectRoot "src\sgoda\automation\workflow_engine"
$TestsDir = Join-Path $ProjectRoot "tests\automation\workflow_engine"
$WorkflowDir = Join-Path $ProjectRoot "automation\n8n\workflows"
$ConfigDir = Join-Path $ProjectRoot "config\automation"
$DocsDir = Join-Path $ProjectRoot "docs\08_Fase_Tecnologica_IV\SPT-019.1"
$ArtifactDir = Join-Path $ProjectRoot "artifacts\technology\SPT-019.1-v1.0.1"
$ReleaseDir = Join-Path $ProjectRoot "releases\SPT-019.1-v1.0.1"

foreach ($Directory in @(
    $SourceDir, $TestsDir, $WorkflowDir, $ConfigDir,
    $DocsDir, $ArtifactDir, $ReleaseDir
)) {
...
$Component = @'
{
  ...
  "scope": [
    ...
    "controlled file-based simulation"
  ],
  "dependencies": [
    "POL-001-v1.0.2"
  ],
  ...
  "n8n_runtime_required_for_tests": false
}
'@
$Policy = @'
{
  "policy_id": "SPT-019.1-POLICY-v1.0.1",
  "component": "SPT-019.1",
  "inline_json_from_powershell_forbidden": true,
  "simulation_event_transport": "UTF-8 JSON file",
  "simulation_must_not_execute_external_actions": true,
  "repository_workflows_must_be_inactive": true,
  "workflow_registry_must_be_deterministic": true
}
'@

Write-Utf8 (Join-Path $SourceDir "__init__.py") $Module
Write-Utf8 (Join-Path $SourceDir "__main__.py") (
    "from . import main" + [Environment]::NewLine +
    "raise SystemExit(main())" + [Environment]::NewLine
)
Write-Utf8 (Join-Path $TestsDir "test_SPT_019_1_core.py") $Tests
Write-Utf8 (Join-Path $ConfigDir "SPT-019.1-component.json") $Component
Write-Utf8 (Join-Path $ConfigDir "SPT-019.1-policy.json") $Policy

Write-Utf8 `
    (Join-Path $WorkflowDir "governance\SPT-019-WF-001-Policy-Audit-Request.json") `
    @'
{
...
Write-Utf8 `
    (Join-Path $WorkflowDir "pmo\SPT-019-WF-002-PMO-Event-Gateway.json") `
    @'
{
...
Write-Utf8 (Join-Path $DocsDir "SPT-019.1-Arquitectura.md") $Architecture
Write-Utf8 (Join-Path $DocsDir "SPT-019.1-Manual-Operativo.md") $Operations

Step "Validando sintaxis Python SPT-019.1"

python -m py_compile `
    "src\sgoda\automation\workflow_engine\__init__.py" `
    "src\sgoda\automation\workflow_engine\__main__.py" `
    "tests\automation\workflow_engine\test_SPT_019_1_core.py"

if ($LASTEXITCODE -ne 0) {
    throw "Falló la validación de sintaxis Python SPT-019.1."
}

Step "Ejecutando pruebas SPT-019.1"
...
if ($LASTEXITCODE -ne 0) {
    throw "Fallaron las pruebas SPT-019.1."
}

Step "Generando registro institucional de workflows"
...
if ($LASTEXITCODE -ne 0) {
    throw "Falló el registro institucional SPT-019.1."
}
...
Step "Ejecutando simulación controlada basada en archivo"

$EventPath = Join-Path $ArtifactDir "simulation-event.json"
$SimulationPath = Join-Path $ArtifactDir "simulation-result.json"

$Event = [ordered]@{
    event = "policy.audit.requested"
    component = "POL-001"
    requested_by = "SPT-019.1"
}

Write-Utf8 `
    $EventPath `
    (($Event | ConvertTo-Json -Depth 20) + [Environment]::NewLine)

$WorkflowPath = Join-Path `
    $WorkflowDir `
    "governance\SPT-019-WF-001-Policy-Audit-Request.json"

Require-Path $WorkflowPath
Require-Path $EventPath
...
Write-Utf8 `
    $SimulationPath `
    (($SimulationOutput -join [Environment]::NewLine) + [Environment]::NewLine)
...
if (-not [bool]$Simulation.simulation) {
    throw "La simulación no fue identificada como simulación."
}

if ([bool]$Simulation.executed) {
    throw "La simulación ejecutó una acción externa, lo cual está prohibido."
}

if ($Simulation.result -ne "accepted_for_simulation") {
    throw "Resultado inesperado de la simulación."
}
...
$Evidence = [ordered]@{
    ...
    n8n_started = $false
    paid_services_required = $false
    candidate_for_integration = $true
...
$Manifest = [ordered]@{
    ...
    tests_approved = $true
    workflow_registry_approved = $true
...
$ReleaseItems = @(
    $SourceDir,
    $TestsDir,
    (Join-Path $WorkflowDir "governance"),
    (Join-Path $WorkflowDir "pmo"),
...
Write-Host "SPT-019.1 v1.0.1 implementado." -ForegroundColor Green
Write-Host "Pruebas: APROBADAS." -ForegroundColor Green
Write-Host (
    "Workflows registrados: " +
    [string]$Registry.workflow_count
) -ForegroundColor Green
Write-Host "Simulación basada en archivo: APROBADA." -ForegroundColor Green
...
Write-Host "Release: releases\SPT-019.1-v1.0.1" -ForegroundColor Cyan
