<#
.SYNOPSIS
    Ejecuta SPT-011A v1.0.2 — Institutional Evidence Closure.
...
$ReleaseDir = Join-Path $ProjectRoot "releases\SPT-011A-v1.0.2"
...
$PreGateJson = Join-Path `
    $EvidenceDir `
    "SPT-011A-v1.0.2-pre-gate-evidence.json"

$PreGateMd = Join-Path `
    $EvidenceDir `
    "SPT-011A-v1.0.2-pre-gate-evidence.md"

$TestEvidenceJson = Join-Path `
    $EvidenceDir `
    "SPT-011A-v1.0.2-test-evidence.json"

$DemoEvidenceJson = Join-Path `
    $EvidenceDir `
    "SPT-011A-v1.0.2-demo-evidence.json"

$GateJson = Join-Path `
    $PmoDir `
    "SPT-011A-v1.0.2-policy-result.json"

$GateMd = Join-Path `
    $PmoDir `
    "SPT-011A-v1.0.2-policy-result.md"

$FinalEvidenceJson = Join-Path `
    $PmoDir `
    "SPT-011A-v1.0.2-implementation-evidence.json"

$FinalEvidenceMd = Join-Path `
    $PmoDir `
    "SPT-011A-v1.0.2-implementation-evidence.md"
...
$CorrectiveDoc = @'
# SPT-011A v1.0.2 — Institutional Evidence Closure
...
$FullSuiteExecuted = -not $SkipFullSuite

if ($SkipFullSuite) {
    $FullSuiteStatus = "skipped_by_operator"
    $FullSuiteMarkdownStatus = "OMITIDA"
}
else {
    $FullSuiteStatus = "passed"
    $FullSuiteMarkdownStatus = "APROBADA"
}

$FullSuiteStartedUtc = $null
$FullSuiteFinishedUtc = $null

if ($null -ne $FullSuiteStarted) {
    $FullSuiteStartedUtc = $FullSuiteStarted.ToString("o")
}

if ($null -ne $FullSuiteFinished) {
...
if (-not $SkipInstitutionalClosure) {
    ...
    Write-Json `
        ...
        -Value ([ordered]@{
            ...
            full_suite_executed = $FullSuiteExecuted
            full_suite_status = $FullSuiteStatus
    ...
    $FinalMarkdown = @"
# SPT-011A v1.0.2 — Cierre institucional
...
- Pruebas específicas: 14 APROBADAS
- Suite completa: $FullSuiteMarkdownStatus
...
Write-Host "SPT-011A v1.0.2 implementado." -ForegroundColor Green
...
if (-not $SkipInstitutionalClosure) {
    ...
    Write-Host "Release: releases\SPT-011A-v1.0.2" -ForegroundColor Cyan
