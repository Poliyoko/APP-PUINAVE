...
$Version = "1.1.9"
$Self = "SGODA-MSYNC-R119-PUBLISH-v3.ps1"
...
$EolPreflightScript = "SGODA-EOL-PREFLIGHT-R119.ps1"
$EolRecoveryScriptV1 = "SGODA-EOL-RECOVERY-R119.ps1"
$EolRecoveryScriptV2 = "SGODA-EOL-RECOVERY-R119-v2.ps1"
$EolPreflightReport = "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/r119-eol-conflicts.json"
$EolRecoveryManifest = "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/eol-recovery-v2/r119-eol-normalization-manifest-v2.json"

$Exact10Unique = @(
    "Install-SPT023.3-Capa1-v1.0.1-OneFile-PS51.ps1",
    "Invoke-SGODA-Untracked-Reconciliation-v2.0.2-PS51.ps1",
    "Prepare-SPT023-Authoritative-Continuation-v1.0.2-PS51.ps1",
    "TRANSFER-Invoke-SGODA-SPT0233-Capa2-FINAL-PS51.ps1",
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/exact-publication-stage-manifest.json",
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/implementation-evidence.json",
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/transaction-recovery-assessment.json",
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/transaction-recovery-sha256-manifest.json",
    "artifacts/development/SPT-020.9-v1.0.0/runs/20260807-021414/backup/docs/00_Estado_Maestro/SGD-000-Estado-Maestro-Institucional-v1.0.0.md",
    "docs/00_Estado_Maestro/ACT-SGODA-MASTER-SYNC-TRANSACTION-RECOVERY-v1.1.9.md"
)

$Exact10ExpectedSha = @{
    "Install-SPT023.3-Capa1-v1.0.1-OneFile-PS51.ps1" = "C7E72B32FD3AEC75BD03F99924C901E1A24DF8C74CD3B6E10CEBBF0B86F3C095"
    "Invoke-SGODA-Untracked-Reconciliation-v2.0.2-PS51.ps1" = "E265CB3429E799ADE061CC342D2A43F46C6E51E2C2F594B20A7E62DA92A1552B"
    "Prepare-SPT023-Authoritative-Continuation-v1.0.2-PS51.ps1" = "D4252365706B2F35BB168213ACFE6B87F5C4F658F1F6C8DCC1B26E694F81D5D9"
    "TRANSFER-Invoke-SGODA-SPT0233-Capa2-FINAL-PS51.ps1" = "140DFC56EF9D34F25ED156A15005E6CF388983338BF2F1A4639E80D12F83BD8F"
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/exact-publication-stage-manifest.json" = "D2FC977229BD05A385548B57A544A5F0031B77B2A3614947E9B0B79A7D17F376"
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/implementation-evidence.json" = "CF774C0B92A59FEFBF64072A4DE62D5C056D7F51FD774B5661F86A087B749A3F"
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/transaction-recovery-assessment.json" = "F016E93015A930D7FAF097FD3C00A7C02C91509F0DB3468A95D011FACE35F904"
    "artifacts/development/SGODA-InstitutionalMasterSynchronization-TRANSACTION-RECOVERY-v1.1.9/transaction-recovery-sha256-manifest.json" = "4D295B12EA8984FFDC1C604101C76F2CE37966515D70465CDAC70CBA424CCC8D"
    "artifacts/development/SPT-020.9-v1.0.0/runs/20260807-021414/backup/docs/00_Estado_Maestro/SGD-000-Estado-Maestro-Institucional-v1.0.0.md" = "3233A0F2F907570FB945FD54146D6FD2DA94E3DDC4B228524E7E60AAD040B874"
    "docs/00_Estado_Maestro/ACT-SGODA-MASTER-SYNC-TRANSACTION-RECOVERY-v1.1.9.md" = "B2800982775CBF1E6DACE6C418A62327E15398DDC99B121CBC4DC9F40B160E0D"
}
...
try {
    ...
    if(-not(Test-Path -LiteralPath (Join-Path $Root $EolRecoveryManifest) -PathType Leaf)){ Hold "R119 EOL recovery manifest missing" }
    $EolManifest=Read-Json $EolRecoveryManifest
    if([int]$EolManifest.remaining_conflicts -ne 0){ Hold "R119 EOL recovery manifest reports remaining conflicts" }
    if([int]$EolManifest.normalized_targets -lt 1){ Hold "R119 EOL recovery manifest has no normalized targets" }
    if([bool]$EolManifest.runtime_staged_preserved -ne $true){ Hold "R119 EOL recovery did not preserve runtime staging" }

    Write-Host "R119_EOL_RECOVERY_MANIFEST=PASS"
    Write-Host "R119_EOL_NORMALIZED_TARGETS=$([int]$EolManifest.normalized_targets)"
    Write-Host "R119_EOL_REMAINING_CONFLICTS=0"

    foreach($P in $Exact10Unique){
        if(-not(Test-Path -LiteralPath (Join-Path $Root $P) -PathType Leaf)){ Hold ("Exact10 missing: "+$P) }
        $Actual10=Get-Sha256 $P
        if($Actual10 -ne $Exact10ExpectedSha[$P]){ Hold ("Exact10 SHA mismatch: "+$P) }
    }

    Write-Host "EXACT10_UNIQUE_COUNT=10"
    Write-Host "EXACT10_SHA256_GATE=PASS"
    Write-Host "EXACT10_TRACKED_EQUIVALENTS=0"
    Write-Host "EXACT10_POLICY=PRESERVE_AND_PUBLISH"
    ...
    # Future-proof reconciliation of historical R119 installer copies.
    # Every earlier root-level R119 .ps1 is preserved as institutional evidence;
    # only the currently executing $Self is excluded here and added later as current output.
    $R119HistoricalFiles=New-Object System.Collections.ArrayList
    foreach($P in $CurrentUntracked){
        if($P -eq $Self){ continue }
        if($P -match '^SGODA-MSYNC-R119(?:-FINAL(?:-v[0-9]+)?)?\.ps1$'){
            if(-not $R119HistoricalFiles.Contains($P)){ [void]$R119HistoricalFiles.Add($P) }
            if(-not $StageBase.Contains($P)){ [void]$StageBase.Add($P) }
        }
    }

    $EolHistoricalFiles=New-Object System.Collections.ArrayList
    foreach($P in $CurrentUntracked){
        if($P -eq $Self){ continue }
        if($P -match '^SGODA-EOL-(?:PREFLIGHT|RECOVERY)-R119(?:-v[0-9]+)?\.ps1$'){
            if(-not $EolHistoricalFiles.Contains($P)){ [void]$EolHistoricalFiles.Add($P) }
            if(-not $StageBase.Contains($P)){ [void]$StageBase.Add($P) }
        }
    }

    foreach($P in @($EolPreflightReport,$EolRecoveryManifest)){
        if(Test-Path -LiteralPath (Join-Path $Root $P) -PathType Leaf){
            if(-not $StageBase.Contains($P)){ [void]$StageBase.Add($P) }
        }
    }
    ...
    # Future-proof all earlier R119 publication scripts as historical evidence.
    foreach($P in $CurrentUntracked){
        if($P -eq $Self){ continue }
        if($P -match '^SGODA-MSYNC-R119(?:-FINAL(?:-v[0-9]+)?|-PUBLISH(?:-v[0-9]+)?)?\.ps1$'){
            if(-not $R119HistoricalFiles.Contains($P)){ [void]$R119HistoricalFiles.Add($P) }
            if(-not $StageBase.Contains($P)){ [void]$StageBase.Add($P) }
        }
    }

    Write-Host "R119_HISTORICAL_SCRIPT_COPIES=$($R119HistoricalFiles.Count)"
    Write-Host "R119_HISTORICAL_SCRIPT_POLICY=PRESERVE_AS_EVIDENCE"
    Write-Host "R119_EOL_SCRIPT_COPIES=$($EolHistoricalFiles.Count)"
    Write-Host "R119_EOL_EVIDENCE_POLICY=PRESERVE_AS_EVIDENCE"
    ...
    foreach($P in $R119HistoricalFiles){
        if(Test-NewTextSecret $P){ Hold ("Secret pattern found in historical R119 script: "+$P) }
    }

    foreach($P in $EolHistoricalFiles){
        if(Test-NewTextSecret $P){ Hold ("Secret pattern found in historical R119 EOL script: "+$P) }
    }

    if(Test-NewTextSecret $Self){ Hold "Secret pattern found in R119 publication script" }
...
"@
    ...
    $PublicationEolConflicts=New-Object System.Collections.ArrayList
    foreach($P in $StageBase){
        if(-not(Test-Path -LiteralPath (Join-Path $Root $P) -PathType Leaf)){ continue }
        $Ext=[IO.Path]::GetExtension($P).ToLowerInvariant()
        if(@(".ps1",".psm1",".psd1",".py",".json",".md",".txt",".yml",".yaml",".toml",".ini",".cfg") -notcontains $Ext){ continue }
        ...
        $T=[IO.File]::ReadAllText((Join-Path $Root $P),[Text.Encoding]::UTF8)
        $Crlf=[regex]::Matches($T,"`r`n").Count
        $BareLf=[regex]::Matches(($T -replace "`r`n",""),"`n").Count

        if(($Required -eq "lf" -and $Crlf -gt 0) -or ($Required -eq "crlf" -and $BareLf -gt 0)){
            [void]$PublicationEolConflicts.Add($P)
        }
    }

    Write-Host "PUBLICATION_EOL_CONFLICTS=$($PublicationEolConflicts.Count)"
    if($PublicationEolConflicts.Count -ne 0){
        $PublicationEolConflicts | ForEach-Object { Write-Host ("EOL_CONFLICT="+$_) }
        Hold "Publication set still contains EOL conflicts"
    }
    Write-Host "PUBLICATION_EOL_GATE=PASS"
    ...
    & git.exe commit -m "fix(institutional): publish R119 master synchronization after final EOL rehash"
    ...
    Write-Host "ATTRIBUTE_DRIVEN_EOL_RECOVERY=PASS"
    Write-Host "EXACT10_RECONCILIATION=PASS"
    Write-Host "EXACT10_POST_EOL_REHASH=PASS"
    Write-Host "EXACT10_POLICY=PRESERVE_AND_PUBLISH"
    Write-Host "SPT025_STATUS=INSTITUTIONALLY_CLOSED"
    ...
    Write-Host "R119_HISTORICAL_SCRIPT_PATTERN_RECONCILIATION=PASS"
    Write-Host "R117_INTEGRITY_MANIFEST=PASS"
    Write-Host "GZIP_RECONSTRUCTION_RECERTIFIED=4"
