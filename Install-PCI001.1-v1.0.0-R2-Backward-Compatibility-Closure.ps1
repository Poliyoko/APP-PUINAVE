...
$ArtifactDir=Join-Path $ProjectRoot "artifacts\consolidation\PCI-001.1-v1.0.0-R2"
$ReportsDir=Join-Path $ArtifactDir "test-reports"
$ReleaseDir=Join-Path $ProjectRoot "releases\PCI-001.1-v1.0.0-R2"
...
def audit(root_value):
    payload = intelligent_audit(root_value)
    findings = []
    for finding in payload["findings"]:
        if finding["code"] == "REPOSITORY_ROOT_MISSING":
            continue
        item = dict(finding)
        if item["code"] == "REGISTRY_CODE_WITHOUT_CANONICAL_DESCRIPTOR":
    ...
    compatible["result"] = "APROBADO" if critical == 0 else "NO APROBADO"
...
def write_outputs(payload, json_path, md_path, html_path):
    ...
    m = payload["metrics"]
    lines = [
        "# SGD-201A.1-R2 — Auditor Inteligente del Índice Maestro",
        "",
...
$Evidence=[ordered]@{program="PCI-SGODA-v1.0.0";increment_code="PCI-001.1";deliverable="SGD-201A.1";version="1.0.0";status="implemented_tested_and_candidate_for_closure";prevalidated_package="..................                                                       [100%] 18 passed in 0.12s";root_cause_corrected=@("policy files excluded","filename fragments rejected","release versions normalized","historical increments classified");intelligent_audit=$Audit;specific_tests=[ordered]@{executed=[int]$Specific.executed;passed=[int]$Specific.passed;failures=[int]$Specific.failures;errors=[int]$Specific.errors;approved=[bool]$Specific.approved};full_suite=[ordered]@{executed=[int]$Full.executed;passed=[int]$Full.passed;failures=[int]$Full.failures;errors=[int]$Full.errors;approved=[bool]$Full.approved};generated_at_utc=[DateTime]::UtcNow.ToString("o")}
...
if($Publish){Step "Publicando mediante gate canónico";& $PublisherPath -Publish -CommitMessage "fix(consolidation): close PCI-001.1 R2 backward compatibility" -EvidenceCommitMessage "chore(consolidation): publish PCI-001.1 R2 evidence";if($LASTEXITCODE-ne 0){throw "Publicación con errores."}}

Step "Resultado final"
Write-Host "PCI-001.1 v1.0.0-R2 implementado." -ForegroundColor Green
Write-Host "SGD-201A.1-R2 — Auditor Inteligente: APROBADO." -ForegroundColor Green
Write-Host "Compatibilidad histórica PCI-001: RESTAURADA." -ForegroundColor Green
Write-Host "code_set histórico: RESTAURADO." -ForegroundColor Green
Write-Host "Alias histórico de hallazgo: RESTAURADO." -ForegroundColor Green
...
Write-Host "Release: releases\PCI-001.1-v1.0.0-R2" -ForegroundColor Cyan
