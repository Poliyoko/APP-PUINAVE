...
def make_template(root,version):
    ...
    (source/'sgoda.template.json').write_text(json.dumps({'schema_version':'1.0','type':'template','name':'versions-template','version':version,'builder_requires':'>=1.11.0,<2.0.0','render_root':'template','variables':{},'files':['template/README.md']}),encoding='utf-8'); return source
