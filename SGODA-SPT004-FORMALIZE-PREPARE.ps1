...
$Version = "1.0.0"
$Self = "SGODA-SPT004-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-004-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt004-real-state-assessment.json"
$CoveragePath = "$Dev/spt004-coverage-matrix.json"
$EvidencePath = "$Dev/spt004-evidence-inventory.json"
$GitPath = "$Dev/spt004-commits-tags-releases.json"
$DependencyPath = "$Dev/spt004-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt004-formalization-prepare.json"
$IntegrityPath = "$Dev/spt004-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"

$DocPath = "docs/06_Tecnologia/SPT-004/SGD-SPT004-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapAssessment = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-003-v1.0.3/spt003-map-reconciliation-assessment.json"
$MapStatus = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-003-v1.0.3/global-deliverable-status-matrix-reconciled.json"
$MapNext = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-003-v1.0.3/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-004 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
try {
    ...
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-004"){
        Hold "Reconciled map does not identify SPT-004 as next audit candidate"
    }

    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-004"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-004 REPOSITORY FOOTPRINT"
    ...
    Write-Host "SPT004_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT004_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT004_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT004_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT004_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT004_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT004_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT004_ACTA_PATHS=$($Actas.Count)"

    if($Direct.Count -lt 1){ Hold "No tracked SPT-004 footprint found" }
    Write-Host "SPT004_DISCOVERY=PASS"
    ...
    $CommitHitsA = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-004" -i)
    $CommitHitsB = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT004" -i)
    ...
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?004|SPT004' })

    Write-Host "SPT004_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT004_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT004_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT004_ACTA_PATHS=$($Actas.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"
    ...
    $Prior = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[1-3]\b' })
    $Later = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[5-7]\b' })

    $CommitAfter004 = @()
    if($CommitSet.Count -gt 0){
        $FirstHash = (($CommitSet[0] -split '\|')[0]).Trim()
        if($FirstHash){
            $CommitAfter004 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20)
        }
    }

    Write-Host "SPT001_TO_003_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT005_TO_007_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT004_COMMIT_SAMPLE=$($CommitAfter004.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-004 REAL STATE"
    ...
    Write-Host "SPT004_REAL_STATE=$RealState"
    Write-Host "SPT004_REASON=$Reason"
    Write-Host "SPT004_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $DependencyObj=[ordered]@{
        component="SPT-004"
        baseline=$ExpectedBaseline
        spt001_to_003_related_paths=$Prior.Count
        spt005_to_007_related_paths=$Later.Count
        post_spt004_commit_sample=@($CommitAfter004)
        sequence_gate="PASS"
    }
    ...
    $PrepareObj=[ordered]@{
        component="SPT-004.FORMALIZE.PREPARE"
        version=$Version
        baseline=$ExpectedBaseline
        real_state=$RealState
    ...
    $Doc=@"
# SPT-004.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo

Auditar el estado historico real de SPT-004 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    Write-Host "SPT-004.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT004_REAL_STATE=$RealState"
    Write-Host "SPT004_RECOMMENDED_ACTION=$RecommendedAction"
