<#
.SYNOPSIS
    Instala SGD-114E v1.0.2 — Native Ecosystem Architecture Policy.
...
$ReleaseDir = Join-Path $ProjectRoot "releases\SGD-114E-v1.0.2"
...
$Doc = @'
# SGD-114E v1.0.2 — Native Ecosystem Architecture Policy
...
foreach ($RootItem in $TerminologyRoots) {
    ...
    foreach ($File in $Files) {
        ...
        foreach ($Rule in $ReplacementRules) {
            $SearchText = [string]$Rule.Search
            $ReplaceText = [string]$Rule.Replace

            if (
                -not [string]::IsNullOrEmpty($SearchText) -and
                $Updated.Contains($SearchText)
            ) {
...
Write-Host "SGD-114E v1.0.2 implementado." -ForegroundColor Green
...
if (-not $SkipInstitutionalClosure) {
    ...
    Write-Host "Release: releases\SGD-114E-v1.0.2" `
