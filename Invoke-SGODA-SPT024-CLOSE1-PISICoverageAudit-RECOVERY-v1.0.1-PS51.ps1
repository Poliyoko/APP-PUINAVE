...
$ArtifactDir="artifacts/development/SPT-024.CLOSE.1-v1.0.1"
...
if($Existing.Count-gt0){Write-Host "CLOSE.1 RESUME MODE : ACTIVE"}else{Write-Host "FAILED MASTER v1.0.0 : LOCAL / SUPERSEDED / NOT PUBLISHED";Write-Host "RECOVERY MASTER v1.0.1 : ACTIVE"}
...
WriteLf $EvidenceFile ([ordered]@{component="SPT-024.CLOSE.1";version="1.0.1";baseline=$ExpectedBaseline;status="PISI_GLOBAL_PREPARE_GATE_PASS";targeted_tests="PASS";institutional_suite="PASS";compileall="PASS";closed_components_preserved=$true;local_remote_sync_required=$true}|ConvertTo-Json -Depth 8)
...
$Allowed=@("Invoke-SGODA-SPT024-CLOSE1-PISICoverageAudit-RECOVERY-v1.0.1-PS51.ps1",$CoreFile,$InitFile,$TestFile,$PolicyFile,$DocFile,$CoverageFile,$ControlMatrixFile,$ReconciliationFile,$AssessmentFile,$IntegrityFile,$PrepareFile,$EvidenceFile)
...
Step 14 "COMMIT"
& git.exe commit -m "fix(spt-024.close.1): recover PISI coverage audit export contract and prepare closure"
