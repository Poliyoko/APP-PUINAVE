<#
.SYNOPSIS
    Instala y ejecuta PCI-002 v1.0.4 — Institutional Consolidation Engine — Canonical Registry Convergence.
...
$ArtifactDir = Join-Path `
    $ProjectRoot `
    "artifacts\consolidation\PCI-002-v1.0.4"
...
$Architecture = @'
# PCI-002 v1.0.4 — Institutional Consolidation Engine — Canonical Registry Convergence
...
try {
    ...
    if ($Publish) {
        ...
        & $PublisherPath `
            -Publish `
            -CommitMessage "feat(consolidation): close PCI-002 v1.0.4 canonical registry convergence" `
            -EvidenceCommitMessage "chore(consolidation): publish PCI-002 v1.0.4 evidence"
    ...
    Write-Host "PCI-002 v1.0.4 implementado." -ForegroundColor Green
...
Write-Host "Convergencia ciclos 2 y 3: APROBADA." -ForegroundColor Green
    ...
    Write-Host "Release: releases\PCI-002-v1.0.4" -ForegroundColor Cyan
