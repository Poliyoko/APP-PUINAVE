...
$ExpectedBaseline = "2c7b9a67b477c2936d8e324739ef6db4813a2d9d"
$Branch = "feature/SPT-001A-rlb-schema-foundation"
$Version = "1.0.0"
$Self = "SGODA-SPT007-FORMALIZE-PREPARE.ps1"

$Dev = "artifacts/development/SPT-007-FORMALIZE-PREPARE-v1.0.0"
$AssessmentPath = "$Dev/spt007-real-state-assessment.json"
$CoveragePath = "$Dev/spt007-coverage-matrix.json"
$EvidencePath = "$Dev/spt007-evidence-inventory.json"
$GitPath = "$Dev/spt007-commits-tags-releases.json"
$DependencyPath = "$Dev/spt007-dependency-sequence-audit.json"
$PreparePath = "$Dev/spt007-formalization-prepare.json"
$IntegrityPath = "$Dev/spt007-prepare-sha256-manifest.json"
$ImplementationEvidencePath = "$Dev/implementation-evidence.json"
$DocPath = "docs/06_Tecnologia/SPT-007/SGD-SPT007-FORMALIZE-PREPARE-Auditoria-Estado-Real.md"

$MapAssessment = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6/spt006-map-reconciliation-assessment.json"
$MapStatus = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6/global-deliverable-status-matrix-reconciled.json"
$MapNext = "artifacts/development/SGODA-DELIVERABLE-MAP-RECONCILE-006-v1.0.6/next-technological-deliverable-assessment-reconciled.json"

function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SPT-007 FORMALIZE PREPARE : HOLD" -ForegroundColor Red
...
function Step { param([int]$N,[string]$Text) Write-Host ""; Write-Host ("[{0}/12] {1}" -f $N,$Text) -ForegroundColor Cyan }
function Git-Fetch {
...
function Read-Json {
    ...
    try { return ([IO.File]::ReadAllText((Resolve-Path -LiteralPath $Path).Path,[Text.Encoding]::UTF8) | ConvertFrom-Json) }
...
function Find-Tracked {
    ...
    foreach($P in $Tracked){
        foreach($Rx in $Patterns){
            if($P -match $Rx){ if(-not $Hits.Contains($P)){ [void]$Hits.Add($P) }; break }
        }
    }
    return @($Hits)
}
function Read-Text {
    param([string]$Path)
    try { return [IO.File]::ReadAllText((Resolve-Path -LiteralPath $Path).Path,[Text.Encoding]::UTF8) }
    catch { return "" }
}
function Contains-AnyMarker {
    param([string]$Path,[string[]]$Markers)
    $Text = Read-Text $Path
    foreach($M in $Markers){ if($Text -match $M){ return $true } }
    return $false
}
...
try {
    ...
    foreach($P in @($MapAssessment,$MapStatus,$MapNext)){ if(-not(Test-Path -LiteralPath $P -PathType Leaf)){ Hold ("Reconciled map input missing: "+$P) } }
    $MapAssessmentObj = Read-Json $MapAssessment
    $MapStatusObj = Read-Json $MapStatus
    $MapNextObj = Read-Json $MapNext
    if([string]$MapNextObj.next_deliverable_candidate -ne "SPT-007"){ Hold "Reconciled map does not identify SPT-007 as next audit candidate" }
    $PendingBefore = @($MapStatusObj.pending_spt_candidates).Count
    if($PendingBefore -ne 28){ Hold ("Expected 28 pending SPT candidates before SPT-007 audit, found "+$PendingBefore) }
    Write-Host "RECONCILED_MAP=PASS"
    Write-Host "MAP_PENDING_SPT_CANDIDATES=28"
    Write-Host "MAP_NEXT_CANDIDATE=SPT-007"
    Write-Host "MAP_NEXT_ACTION=AUDIT_NEXT_PENDING_DELIVERABLE"

    Step 3 "DISCOVER SPT-007 REPOSITORY FOOTPRINT"
    $Tracked = @(& git.exe -c core.quotepath=false ls-files)
    $Direct = Find-Tracked @('(?i)(^|/)SPT-007([^0-9]|$)','(?i)(^|/)SPT007([^0-9]|$)','(?i)\bSPT[-_.]?007\b')
    $ProgramRelated = Find-Tracked @('(?i)(^|/)SPT-0(0[1-9]|1[0-9])([^0-9]|$)','(?i)\bSPT[-_.]?0(0[1-9]|1[0-9])\b')
    ...
    Write-Host "SPT007_TRACKED_PATHS=$($Direct.Count)"
    Write-Host "SPT007_TEST_PATHS=$($Tests.Count)"
    Write-Host "SPT007_DOC_PATHS=$($Docs.Count)"
    Write-Host "SPT007_ARTIFACT_PATHS=$($Artifacts.Count)"
    Write-Host "SPT007_SCRIPT_PATHS=$($Scripts.Count)"
    Write-Host "SPT007_SOURCE_PATHS=$($Source.Count)"
    Write-Host "SPT007_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT007_ACTA_PATHS=$($Actas.Count)"
    if($Direct.Count -lt 1){ Hold "No tracked SPT-007 footprint found" }
    Write-Host "SPT007_DISCOVERY=PASS"

    Step 4 "CLOSURE / IMPLEMENTATION / PREPARE MARKER AUDIT"
    $ClosureMarkers = @('(?i)\bCLOSED\b','(?i)INSTITUTIONALLY[_ ]CLOSED','(?i)TECHNICALLY[_ ]CLOSED','(?i)\bCIERRE\b','(?i)\bCERRADO\b','(?i)FINAL_EXIT_CODE\s*=\s*0','(?i)CLOSURE[_ ]GATE\s*=\s*PASS','(?i)\bOFFICIALLY[_ ]PUBLISHED\b')
    $ImplementationMarkers = @('(?i)\bPASS\b','(?i)\bIMPLEMENTED\b','(?i)\bFINAL\b','(?i)\bAPPROVED\b','(?i)TARGETED_TESTS\s*=\s*PASS','(?i)FULL_SUITE\s*=\s*PASS','(?i)COMPILEALL\s*=\s*PASS')
    $PrepareMarkers = @('(?i)\bPREPARE\b','(?i)NEXT_ACTION','(?i)READY_FOR')
    ...
    $CommitHitsA = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT-007" -i)
    $CommitHitsB = @(& git.exe --no-pager log --all --format="%H|%ad|%s" --date=iso --grep="SPT007" -i)
    $CommitSet = New-Object System.Collections.ArrayList
    foreach($C in @($CommitHitsA + $CommitHitsB)){ if($C -and -not $CommitSet.Contains($C)){ [void]$CommitSet.Add($C) } }
    $Tags = @(& git.exe tag --list)
    $TagHits = @($Tags | Where-Object { $_ -match '(?i)SPT[-_.]?007|SPT007' })
    Write-Host "SPT007_COMMIT_HITS=$($CommitSet.Count)"
    Write-Host "SPT007_TAG_HITS=$($TagHits.Count)"
    Write-Host "SPT007_RELEASE_PATHS=$($Releases.Count)"
    Write-Host "SPT007_ACTA_PATHS=$($Actas.Count)"
    Write-Host "TAG_CREATED=NO"
    Write-Host "RELEASE_CREATED=NO"
    ...
    $EvidenceCandidates = @($Artifacts + $Releases + $Actas + ($Direct | Where-Object { $_ -match '(?i)evidence|assessment|manifest|ledger|acta|report|resultado|result|audit|certif' })) | Select-Object -Unique
    $PassEvidence = New-Object System.Collections.ArrayList
    foreach($P in $EvidenceCandidates){
        if(Contains-AnyMarker $P @('(?i)\bPASS\b','(?i)FINAL_EXIT_CODE\s*=\s*0','(?i)\bAPPROVED\b','(?i)\bCLOSED\b')){ [void]$PassEvidence.Add($P) }
    }
    ...
    $Prior = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?00[1-6]\b' })
    $Later = @($ProgramRelated | Where-Object { $_ -match '(?i)SPT[-_.]?0(0[8-9]|1[0-9])\b' })
    $CommitAfter007 = @()
    if($CommitSet.Count -gt 0){
        $FirstHash = (($CommitSet[0] -split '\|')[0]).Trim()
        if($FirstHash){ $CommitAfter007 = @(& git.exe --no-pager log --all --format="%H|%s" "$FirstHash..HEAD" -20) }
    }
    Write-Host "SPT001_TO_006_RELATED_PATHS=$($Prior.Count)"
    Write-Host "SPT008_TO_019_RELATED_PATHS=$($Later.Count)"
    Write-Host "POST_SPT007_COMMIT_SAMPLE=$($CommitAfter007.Count)"
    Write-Host "DEPENDENCY_SEQUENCE_AUDIT=PASS"

    Step 8 "DETERMINE SPT-007 REAL STATE"
    ...
    Write-Host "SPT007_REAL_STATE=$RealState"
    Write-Host "SPT007_REASON=$Reason"
    Write-Host "SPT007_RECOMMENDED_ACTION=$RecommendedAction"
    ...
    $CoverageObj=[ordered]@{component="SPT-007";version=$Version;baseline=$ExpectedBaseline;generated_at=$Now;tracked_paths=@($Direct);tests=@($Tests);docs=@($Docs);artifacts=@($Artifacts);scripts=@($Scripts);source=@($Source);releases=@($Releases);actas=@($Actas);closure_marker_files=@($ClosureFiles);implementation_marker_files=@($ImplementationFiles);prepare_marker_files=@($PrepareFiles)}
    $EvidenceObj=[ordered]@{component="SPT-007";real_state=$RealState;evidence_candidates=@($EvidenceCandidates);pass_evidence_files=@($PassEvidence);closure_markers=@($ClosureFiles);implementation_markers=@($ImplementationFiles);prepare_markers=@($PrepareFiles);commit_history=@($CommitSet);tag_history=@($TagHits);release_paths=@($Releases);acta_paths=@($Actas);destructive_cleanup=$false;repository_write=$false}
    $GitObj=[ordered]@{component="SPT-007";baseline=$ExpectedBaseline;commits=@($CommitSet);tags=@($TagHits);release_paths=@($Releases);acta_paths=@($Actas);tag_created=$false;release_created=$false;policy="AUDIT_EXISTING_ONLY"}
    $DependencyObj=[ordered]@{component="SPT-007";baseline=$ExpectedBaseline;spt001_to_006_related_paths=$Prior.Count;spt008_to_019_related_paths=$Later.Count;post_spt007_commit_sample=@($CommitAfter007);sequence_gate="PASS"}
    $AssessmentObj=[ordered]@{component="SPT-007";version=$Version;baseline=$ExpectedBaseline;real_state=$RealState;reason=$Reason;recommended_action=$RecommendedAction;tracked_path_count=$Direct.Count;test_path_count=$Tests.Count;evidence_count=$EvidenceCandidates.Count;pass_evidence_count=$PassEvidence.Count;closure_marker_count=$ClosureFiles.Count;implementation_marker_count=$ImplementationFiles.Count;prepare_marker_count=$PrepareFiles.Count;commit_hit_count=$CommitSet.Count;tag_hit_count=$TagHits.Count;release_path_count=$Releases.Count;acta_path_count=$Actas.Count;destructive_cleanup=$false;new_functionality=$false;production_change=$false}
    $PrepareObj=[ordered]@{component="SPT-007.FORMALIZE.PREPARE";version=$Version;baseline=$ExpectedBaseline;real_state=$RealState;next_action=$RecommendedAction;if_already_closed="RECONCILE_MASTER_MAP_WITHOUT_REOPENING";if_pending_formalization="BUILD_FORMALIZATION_CLOSURE_PACKAGE";if_incomplete="IDENTIFY_AND_COMPLETE_MISSING_ELEMENTS";commit_allowed=$false;push_allowed=$false;tag_allowed=$false;release_allowed=$false;destructive_cleanup=$false}
    $ImplementationObj=[ordered]@{component="SPT-007.FORMALIZE.PREPARE";version=$Version;baseline=$ExpectedBaseline;audit_mode="READ_ONLY_REPOSITORY_AUDIT_WITH_LOCAL_EVIDENCE_OUTPUT";map_candidate="SPT-007";map_pending_before=28;real_state=$RealState;recommended_action=$RecommendedAction;git_add=$false;commit=$false;push=$false;tag=$false;release=$false;destructive_cleanup=$false}
    ...
    $Doc=@"
# SPT-007.FORMALIZE.PREPARE

Linea base: $ExpectedBaseline

## Objetivo
Auditar el estado historico real de SPT-007 antes de cualquier formalizacion, reapertura o desarrollo adicional.
...
"@
    ...
    $Outputs=@($AssessmentPath,$CoveragePath,$EvidencePath,$GitPath,$DependencyPath,$PreparePath,$ImplementationEvidencePath,$DocPath)
    $Records=@()
    foreach($P in $Outputs){ $Records += [ordered]@{path=$P;sha256=Get-Sha256 $P} }
    $ManifestObj=[ordered]@{component="SPT-007.FORMALIZE.PREPARE";version=$Version;baseline=$ExpectedBaseline;records=$Records}
    ...
    Write-Host "SPT-007.FORMALIZE.PREPARE : CLOSED / PASS" -ForegroundColor Green
    Write-Host "SPT007_REAL_STATE=$RealState"
    Write-Host "SPT007_RECOMMENDED_ACTION=$RecommendedAction"
