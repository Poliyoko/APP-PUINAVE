...
if (-not (Test-Path $modulePath)) {
    throw "No se encontró el módulo UTF-8: $modulePath"
}

$backupDir = Join-Path $root "artifacts\pmo\SPB-005.3-UTF8-REPOSITORY-STANDARD\patch-backups"
New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
Copy-Item $modulePath (Join-Path $backupDir "repository_utf8.py.before-v1.1") -Force
...
def repair_mojibake(text: str) -> tuple[str, bool]:
    """
    Repara únicamente transformaciones reversibles típicas UTF-8 -> CP1252.

    La reparación se acepta solo si:
    - la conversión es válida;
    - reduce el número de marcadores de mojibake;
    - no introduce el carácter de reemplazo.
    """
    ...
    best_score = count_mojibake(best)

    if best_score < current_score and "�" not in best:
        return best, True
...
def normalize_file(path: Path, backup_root: Path, root: Path) -> dict[str, str]:
    ...
    try:
        text = data.decode("utf-8-sig")
        source_encoding = "utf-8-bom" if data.startswith(b"\xef\xbb\xbf") else "utf-8"
    ...
    if apply:
        for item in before:
            if item.status != "NON_COMPLIANT":
                continue
            normalized.append(
                normalize_file(root / item.path, backup_root, root)
            )
...
Write-Host ""
Write-Host "Ejecutando normalización UTF-8 v1.1..."
...
Write-Host ""
Write-Host "Parche SPB-005.3 v1.1 aplicado."
