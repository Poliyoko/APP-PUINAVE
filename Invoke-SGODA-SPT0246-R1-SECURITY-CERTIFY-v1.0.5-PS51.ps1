...
$SelfName = "Invoke-SGODA-SPT0246-R1-SECURITY-CERTIFY-v1.0.5-PS51.ps1"
...
function IsTx([string]$P) {
    ...
    if ($p -match '^Invoke-SGODA-SPT0246-Capa1-FINAL-v[0-9]+\.[0-9]+\.[0-9]+-PS51\.ps1$') { return $true }
...
'@
    Write-Host "ASSESSMENT MODE       : GIT-TRACKED STATIC SCOPE"
    Write-Host "ASSESSMENT EXECUTION  : START"
    try {
        Write-Lf $probe $pc
        Native $Py @($probe) "Production client security assessment"
    }
    finally {
        Remove-Item $probe -Force -ErrorAction SilentlyContinue
    }
    if (-not (Test-Path -LiteralPath $AssessmentFile)) { Stop-Hold "Client security assessment report was not created." }
