...
$SelfName = "Invoke-SGODA-SPT0242-R1-CERTIFY-v1.0.1-PS51.ps1"
$CommitMessage = "feat(spt-024.2): remediate and certify secrets security gate"
$TargetedExpected = 29
$InstitutionalFloor = 1248
$CommitCreated = $false
...
'@
    ...
    # Re-evaluate after remediation. The original report lists patterns missing
    # before apply(), so it cannot itself certify the post-remediation state.
    $gitignoreNow = Get-Content -LiteralPath (Join-Path $Root ".gitignore") -Raw -Encoding UTF8
    $requiredGitignore = @(".env",".env.*","*.pem","*.key","*.pfx","*.p12")
    $missingAfter = @(
        $requiredGitignore | Where-Object {
            $pattern = $_
            -not (($gitignoreNow -split "`r?`n") -contains $pattern)
        }
    )
    $gitignorePassed = ($missingAfter.Count -eq 0)
    Write-Host "GITIGNORE CONTROL         : $(if ($gitignorePassed) { 'PASS' } else { 'HOLD' })"
    ...
    if ($confirmed -gt 0 -or $review -gt 0 -or -not $gitignorePassed) {
        Write-Host ""
        ...
        Fail (
            ...
            "Confirmed risks=$confirmed; Review required=$review; Gitignore passed=$gitignorePassed. " +
