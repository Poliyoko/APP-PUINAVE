...
$EvidenceRoot = "artifacts/development/SGODA-InstitutionalMasterSynchronization-RECOVERY-v1.1.2"
...
$RecoveryDoc = "docs/00_Estado_Maestro/SGODA-PUINAVE-Sincronizacion-Institucional-Maestra-RECOVERY-v1.1.2.md"
$RecoveryAct = "docs/00_Estado_Maestro/ACT-SGODA-MASTER-SYNC-RECOVERY-v1.1.2.md"
...
function Hold {
    param([string]$Reason)
    Write-Host ""
    Write-Host "SGODA MASTER SYNCHRONIZATION RECOVERY v1.1.2 : HOLD" -ForegroundColor Red
...
try {
    ...
    Step 5 "REFRESH EXACT PRESERVATION SET / ACCEPT PREPARE OUTPUTS"
    ...
    $PrepareOutputsAccepted = New-Object System.Collections.ArrayList
    $SupersededRecoveryCopies = New-Object System.Collections.ArrayList

    $RequiredPrepareOutputs = @($ManifestPath,$LedgerPath,$PreparePath)
    $KnownReconciliationTools = @(
    ...
    foreach($P in $CurrentUntracked) {
        if($P -eq "SGODA-RECOVERY-112.ps1") {
            [void]$Preserve.Add($P)
            continue
        }
        ...
        if($RequiredPrepareOutputs -contains $P) {
            [void]$Preserve.Add($P)
            [void]$PrepareOutputsAccepted.Add($P)
            continue
        }
        ...
        if($KnownReconciliationTools -contains $P) {
            [void]$Preserve.Add($P)
            continue
        }

        # Browser/download copies of the already superseded long-name v1.1.0 RECOVERY
        # are historical execution evidence. Preserve them; never execute them here.
        if($P -match '^Invoke-SGODA-InstitutionalMasterSynchronization-SPT025Reconciliation-RECOVERY-v1\.1\.0-PS51.*\.ps1$') {
            [void]$Preserve.Add($P)
            [void]$SupersededRecoveryCopies.Add($P)
            continue
        }

        # Short-name recovery files from previous attempts are also execution evidence.
        # v1.1.0 and v1.1.1 are superseded by this v1.1.2 transaction.
        if($P -match '^SGODA-RECOVERY-11[01]\.ps1$') {
            [void]$Preserve.Add($P)
            [void]$SupersededRecoveryCopies.Add($P)
            continue
        }
    ...
    Write-Host "PREPARE_OUTPUTS_ACCEPTED=$($PrepareOutputsAccepted.Count)"
    Write-Host "SUPERSEDED_RECOVERY_COPIES=$($SupersededRecoveryCopies.Count)"
    Write-Host "UNEXPECTED_UNTRACKED=$($Unexpected.Count)"

    if($PrepareOutputsAccepted.Count -ne 3) {
        Hold "The three PREPARE reconciliation JSON outputs are not all present"
    }

    if($Unexpected.Count -ne 0) {
        $Unexpected | ForEach-Object { Write-Host ("UNEXPECTED=" + $_) }
        Hold "Untracked set contains genuinely new content outside the refreshed reconciliation policy"
    }

    Write-Host "PREPARE_OUTPUT_RECONCILIATION=PASS"
    Write-Host "SUPERSEDED_RECOVERY_FILES=PRESERVED_AS_EVIDENCE"
    ...
    $EvidenceObject = [ordered]@{
        ...
        prepare_outputs_accepted = $PrepareOutputsAccepted.Count
        superseded_recovery_copies = $SupersededRecoveryCopies.Count
        institutional_tests = $Passed
    ...
    $DocText = @"
# SGODA-PUINAVE - Sincronizacion Institucional Maestra - RECOVERY v1.1.2
...
"@
    $ActText = @"
# ACT-SGODA-MASTER-SYNC-RECOVERY-v1.1.2
...
"@
    ...
    & git.exe commit -m "fix(institutional): complete master synchronization worktree reconciliation recovery v1.1.2"
    ...
    Write-Host "PREPARE_OUTPUT_RECONCILIATION=PASS"
    Write-Host "SUPERSEDED_RECOVERY_FILES=PRESERVED"
    ...
    Write-Host ""
    Write-Host "SGODA INSTITUTIONAL MASTER SYNCHRONIZATION RECOVERY v1.1.2 : CLOSED / PASS" -ForegroundColor Green
