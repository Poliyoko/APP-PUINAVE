...
$ExpectedRuntimeSha = "B61F1B3904349C1286B4C9EEFCA3392C6919B03B6BEA9D7D3E14BBDC84B8B60F"
...
try {
    ...
    if($RuntimeModified.Count -ne 1){ Hold "Expected runtime state modification is not present" }

    $RuntimeSha=Get-Sha256 $RuntimeState
    if($RuntimeSha -ne $ExpectedRuntimeSha){ Hold "Runtime state SHA-256 changed from audited value" }
    ...
    Write-Host "RUNTIME_STATE_MODIFIED=YES"
...
"@
    ...
    $StageManifestObj=[ordered]@{
        ...
        runtime_tracked_ignored_path=$RuntimeState;runtime_stage_mode="git add -u -- tracked path";
    ...
    Step 10 "EXACT CONTROLLED STAGING / TRACKED RUNTIME RECOVERY"
    ...
    # Runtime state is already tracked. Stage only the tracked modification.
    # This does not force-add a new ignored path and does not alter .gitignore.
    & git.exe -c core.autocrlf=false -c core.eol=lf -c core.safecrlf=false add -u -- $RuntimeState
    if($LASTEXITCODE -ne 0){ Hold "git add -u failed for tracked runtime state" }
